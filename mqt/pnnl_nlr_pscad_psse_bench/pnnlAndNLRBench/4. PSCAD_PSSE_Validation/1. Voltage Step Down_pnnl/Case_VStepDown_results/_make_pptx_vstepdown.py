"""Build the Case 1 (Voltage Step Down) PowerPoint deck.

Inherits theme from slide_templates/meeting_update_042226_r3.pptx and writes
one plot per slide using PNGs produced by _plot_vstepdown.py.
"""
import datetime
import glob
import os
...
# Slide canvas defaults (will be overridden per-template below).
SLIDE_W_IN = 10.0
SLIDE_H_IN = 5.625

CASE_TITLE = "Case 1: Voltage Step Down from 1.0 p.u. to 0.95 p.u."
CASE_SUBTITLE = "PNNL REGFM_A1 — PSCAD"

# Per-plot slide titles + base name (timestamp suffix discovered at runtime).
PLOTS = [
    ("V (pu)",    "v_pu"),
    ("I (pu)",    "i_pu"),
    ("P (pu)",    "p_pu"),
...
def latest_run_ts():
    """Pick the run timestamp the plot script just wrote."""
    ...
    if os.path.exists(marker):
        ...
        if ts:
            return ts
    # Fallback: newest timestamped v_pu_*.png
    cands = glob.glob(os.path.join(PLOT_DIR, "v_pu_*.png"))
    if not cands:
        return None
    cands.sort(key=os.path.getmtime, reverse=True)
    base = os.path.basename(cands[0])  # v_pu_YYYYMMDD_HHMMSS.png
...
def clear_all_placeholders(slide):
    """Remove every placeholder so the slide is a blank canvas."""
...
def add_custom_title(slide, text, *, top_in=0.25, height_in=0.70, size=18,
                     left_pad=0.4):
    """Place a custom title textbox at a known-safe location.

    Bypasses the template placeholder (which clips descenders against the
    slide edge in the meeting_update_042226_r3 template).
    """
...
def add_title_slide(prs, title, subtitle):
    ...
    # Title — vertically centered region of the slide
...
def add_plot_slide(prs, plot_title, img_path):
    ...
    # Image area sits below the custom title (top 0.25 + height 0.70 + gap)
    content_top = 1.10
    content_h_max = SLIDE_H_IN - content_top - 0.30

    # Try a wide width first; if too tall, scale down by height instead.
    ...
    if h_at_max_w <= content_h_max:
        ...
        # height-bound: shrink width proportionally
...
def main():
    ...
    # Adopt template's actual slide dimensions (covers 16:9 vs widescreen).
    ...
    out = os.path.join(HERE, f"Case1_VoltageStepDown_PNNL_REGFMA1_{ts}.pptx")
    prs.save(out)
