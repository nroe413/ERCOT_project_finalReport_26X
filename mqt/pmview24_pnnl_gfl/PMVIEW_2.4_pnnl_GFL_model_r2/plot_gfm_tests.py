#!/usr/bin/env python3
"""
plot_gfm_tests.py - Plot PSCAD PMView simulation outputs for PNNL GFL MQT tests.
...
def plot_overlay(time, data, channels, title, save_path):
    ...
    # Use Vrms_gridSide_LCL if available, otherwise fall back to Vsource
    idx_vrms = find_channel_by_desc(channels, "Vrms_gridSide_LCL")
    vrms_label = r"$V_{rms,grid}$"
    if idx_vrms is None:
        idx_vrms = find_channel_by_desc(channels, "Vsource")
        vrms_label = r"$V_{source}$"
    ...
    # Secondary axis: voltage (fixed 0.8–1.2 pu)
    ax2 = ax1.twinx()
    l3, = ax2.plot(t, d[:, idx_vrms], label=vrms_label,
...
def plot_test(test_num):
    ...
    for fname, title, ch_list, ylabel in PLOT_GROUPS:
        ...
        plot_group(time, data, indices,
                   f"{title} \u2014 PNNL GFL \u2014 Test {test_num} ({profile})",
                   ylabel, save_path)

    # Generate overlay plot (Ppoi, Qpoi on left axis; voltage on right)
    overlay_path = os.path.join(out_dir, "pq_vrms_overlay.png")
    plot_overlay(time, data, channels,
                 f"POI Power & Grid Voltage \u2014 PNNL GFL \u2014 Test {test_num} ({profile})",
                 overlay_path)
