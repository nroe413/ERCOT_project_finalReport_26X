...
Energy-function methods offer a bridge from transient stability analysis of synchronous machines to GFM power systemes \cite{sauer1998,pai1989,athay1979,bergen1981,chiang1988}. These methods compare an energy-like measure of the post-fault state with an energy boundary associated with unstable equilibria. Energy functions quantify the total energy of a system as the sum of kinetic and potential energies contributed from generators.  Applying energy functions to GFM systems requires attention to the assumptions behind the power-angle relationship and damping representation.
...
\label{eq:fi}
\end{equation}
with $P_{\mathrm{COI}}=\sum_i(P_{\mathrm{m},i}-P_{\mathrm{e},i})$ and $M_{\mathrm{T}}=\sum_i M_i$. Here $P_{\mathrm{m},i}$ is the mechanical power of a machine or the power reference $p^{*}$ of an inverter, and $G_{ij}$ and $B_{ij}$ are the conductance and susceptance ($B=\operatorname{Im}\{1/Z\}$,
$G=\operatorname{Re}\{1/Z\}$) of the reduced network between internal nodes $i$ and $j$; in the 39-bus cases the electrical power is taken from the constant-power network solution instead of a reduced admittance matrix, with the same dependence on the internal voltages and the coupling reactances. So $f_i$, and with it the potential energy $V_{\mathrm{PE}}$ and the shape of the surfaces in Figures~\ref{fig:esbelow} and~\ref{fig:esat}, contain the dispatch, the internal voltages, and the network admittances only. 

The inertia coefficients enter the kinetic term and the center-of-inertia weighting $M_i/M_{\mathrm{T}}$. Damping acts on how $V$ changes along a trajectory. $V$ is a Lyapunov function describing a stable system if  $V$ is greater than or equal to $0$  about the origin, $(0,0)$ in a system with two state variables, and whose derivative with respect to time along all possible trajectories, $\dot V(x)$,  is negative or equal to zero at points other than the origin, if the system is stable \cite{khalil2002}. 

The energy functions for machines use $D=0$. For the GFM system, the damping $D_i=1/m_{\mathrm{p}}$ is retained for energy function. For the single-machine model, $V_{\mathrm{cr}}=V_{\mathrm{PE}}(\delta_{\mathrm{u}})$.

For each synchronous machine and GFM system tested, the transient stability is quantified with the critical clearing time metric. Critical clearing time captures the maximum fault duration that a power system can withstand before losing a specified stability criterion. Stability is reflected in the systems' generators rotor angle or inverter angle behavior with respect to time. 

The critical clearing time is calculated in three steps:
\begin{enumerate}
\item Compute $\theta^{\mathrm{s}}$.
