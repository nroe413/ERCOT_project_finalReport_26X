...
The testbed combines configurable network disturbances, electric generator models, and scripts that process the simulation outputs. Eleven configurations replace zero through ten synchronous machines with the selected droop-controlled GFM model; the testbed also supports later studies of other GFM controls, grid-following (GFL) resources, mixed generation, user-defined load profiles, and load disturbances. The results shown are specific to the GFM model and the test cases: one droop-controlled GFM implementation and a limited set of disturbances at several locations within the 39-bus network. Other GFM controls, other settings of this one, or other scenarios may behave differently.

The reference simulations show fault-ride-through recovery of the connected network in every generation configuration, from the all-machine system to the all-GFM system. The selected GFM responses settle faster than the corresponding machine responses after the three-phase-to-ground fault. 

The analytical comparison connects these observations to familiar machine dynamics. A synchronous machine's rotor motion sets its electrical phase angle. A GFM controller generates its electrical phase angle without a rotating shaft. Under unsaturated filtered droop, the synchronous machine and GFM dynamic models can be written in the same reduced swing-equation form. The GFM droop gain sets an equivalent damping coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}$, while the combination of the power-measurement filter and droop gain sets the GFM equivalent inertia. For the selected parameters, the analytical low-frequency damping coefficient of the GFM is 100 per unit, five times the 20 per unit given by the machine's governor droop, on the corresponding normalized bases.
...
The next phase of the research collaboration will align topology, operating points, ratings, bases, applicable controls, and disturbances between corresponding PSS/E and PSCAD studies. The objective is to understand differences between modeling domains. We will also extend the preliminary analytical work to investigate how current limits, voltage limits, and droop settings influence transient stability of power systems. Together these simulation and analysis-focused research thrusts will provide a foundation for guidance to manufacturers and power system engineers.
...
Three terms connect GFM controls to the machine-based interpretation developed in this report. Mathematical equivalence of the GFM and synchronous-machine dynamics does not imply physical equivalence of inverter-based resources and synchronous generators.
\begin{enumerate}
...
\end{tikzpicture}
\caption{Testbed workflow: E-TRAN converts the IEEE 39-bus PSS/E case into a PSCAD EMT network; the ERCOT-supplied synchronous-machine dynamics and the PNNL REGFM\_A1 grid-forming inverter dynamics are inserted into that network.}
...
We selected a short reference disturbance to represent a practical protection timescale. The 0.0833~s duration corresponds to approximately five cycles at 60~Hz and is applied from 3.0~s. After establishing the machine baseline fault-ride-through behavior, we replaced machines with GFM units dispatched at the same active power and voltage operating points as the machines. Because the GFM units here regulate voltage, we did not fix a reactive-power operating point; the $Q$--$V$ droop of each unit supplies reactive power according to its voltage reference. The resulting reactive outputs are close to the machines' but are not set equal to them. We replaced the synchronous machines one at a time and reapplied the same reference five-cycle, three-phase-to-ground fault. Extended-duration tests discussed later explore transient stability limits rather than recommended protection operating times. Table~\ref{tab:cases} lists the simulated cases. Network faults were applied at buses~14, 13, 10, and~39. All faults in the replacement progression and the current-limit studies are three-phase-to-ground. Single-line-to-ground and double-line-to-ground faults at bus~14 are reported in Section~\ref{sec:unbal}. Instantaneous failures of the semiconductor switches, which occur on a much faster timescale than the control limiter, are not represented in any case. The grid-following model is the only one with explicit switch elements.

\begin{table}[tbp]\centering\small
\caption{Simulated fault cases. Fault resistance 0.01~$\Omega$ in the 39-bus cases and 0.001~$\Omega$ in the single-device cases; fault applied at 3.0~s.}
\label{tab:cases}
...
\caption{Bus-10 current-limit study with a five-cycle fault and no reclosing: each trace is the inverter's own current magnitude at its terminals, on the 10~kV side of its step-up transformer, in per unit of the unit rating.}
\label{fig:peakits}
\end{figure}

Figure~\ref{fig:ilimarms} compares the bus-14 reference fault on the all-GFM fleet at $I_{\max F}=2.0$, 1.5, 1.2, and 1.1 per unit with all other settings unchanged; the last two are sensitivity points well below the 2.0 per-unit default. Table~\ref{tab:ilimarms} collects the peak currents and the limiter timing of these cases and of the bus-13 cases that follow. The fault is electrically remote from the units: at 2.0 per unit no unit reaches its limit, with peaks of 1.53, 1.44, and 1.38 per unit at buses~32, 30, and~31 and the other seven below 0.92 per unit. At 1.5 per unit only the bus-32 unit touches the limit, for 1.4~ms. At 1.2 per unit the three nearest units exceed the limit in the internal current-magnitude signal, with peaks of 1.44, 1.38, and 1.31 per unit at buses~32, 30, and~31; the current is back at the limit 0.8 to 0.9 cycle after fault inception at buses~30 and~31 and 1.7 cycles after inception at bus~32, whose current makes a second, smaller excursion; the total time above the limit is 0.4 to 0.8 cycle. At 1.1 per unit the same three units peak at 1.43, 1.30, and 1.28 per unit and are back at the limit after 1.8, 0.9, and 0.9 cycles; unit~32's current averages 0.94 per unit over the third to fifth cycles of the fault, against 1.05 at 2.0 per unit. Instantaneous failures of the semiconductor switches, which occur on a much faster timescale than the limiter's correction, are not modeled: the units stay connected and the grid-forming controller's current limiter function alone bounds the current. In all four cases every unit rides through the fault. Post-fault active power returns to within 0.5\% of its pre-fault value and the frequency settles at 60.01~Hz.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{narr_ilim_arms_bus14}
\caption{Bus-14 reference fault on the all-GFM fleet at four transient current limits. Left: peak of each unit's internal current magnitude, with the limit lines; right: the same signal for the bus-32 unit through the fault. }
\label{fig:ilimarms}
\end{figure}

\begin{table}[tbp]\centering\small
\caption{Current-limit cases on the all-GFM fleet (Figures~\ref{fig:ilimarms} and~\ref{fig:ilimarms13}): the units at buses~30, 31, and~32, the three nearest the fault. Peak current is the maximum of the inverter's internal current magnitude. Time above limit is the total time the current spends above the limit within one second of fault inception. Return to limit counts from fault inception, not from the moment the limit is first exceeded: it is the time until the current is within 2\% of the limit and stays there for a full cycle. A dash means the limit was not reached; the 15 per-unit case has no practical limit. The other seven units stay below 0.92 per unit in every case.}
\label{tab:ilimarms}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{cc*{9}{>{\centering\arraybackslash}p{0.9cm}}c}\toprule
 & & \multicolumn{3}{c}{\shortstack{Peak current\\(p.u.)}} & \multicolumn{3}{c}{\shortstack{Time above limit\\(cycles, total)}} & \multicolumn{3}{c}{\shortstack{Return to limit\\(cycles from fault)}} & \\
\cmidrule(lr){3-5}\cmidrule(lr){6-8}\cmidrule(lr){9-11}
Fault bus & $I_{\max F}$ & 30 & 31 & 32 & 30 & 31 & 32 & 30 & 31 & 32 & Rides through\\\midrule
14 & 2.0 & 1.44 & 1.38 & 1.53 & -- & -- & -- & -- & -- & -- & yes\\
14 & 1.5 & 1.41 & 1.35 & 1.51 & -- & -- & 0.08 & -- & -- & 0.67 & yes\\
14 & 1.2 & 1.38 & 1.31 & 1.44 & 0.50 & 0.40 & 0.80 & 0.91 & 0.84 & 1.69 & yes\\
14 & 1.1 & 1.30 & 1.28 & 1.43 & 0.55 & 0.65 & 1.12 & 0.94 & 0.89 & 1.78 & yes\\\midrule
13 & 15 & 1.30 & 1.43 & 1.83 & -- & -- & -- & -- & -- & -- & yes\\
13 & 2.0 & 1.30 & 1.43 & 1.69 & -- & -- & -- & -- & -- & -- & yes\\
13 & 1.5 & 1.30 & 1.41 & 1.63 & -- & -- & 0.37 & -- & -- & 0.84 & yes\\
13 & 1.2 & 1.28 & 1.35 & 1.59 & 0.34 & 0.42 & 1.03 & 0.83 & 0.85 & 1.75 & yes\\
13 & 1.1 & 1.24 & 1.31 & 1.59 & 0.46 & 0.52 & 1.37 & 0.89 & 0.90 & 1.82 & yes\\
13 & 1.0 & 1.15 & 1.24 & 1.44 & 0.54 & 0.62 & 1.81 & 0.92 & 0.97 & 1.80 & no\\\bottomrule
\end{tabular}
\end{table}

Because the bus-14 fault is remote, Figure~\ref{fig:ilimarms13} repeats the comparison with the fault at bus~13, one short line from bus~10, the point of interconnection of unit~32, and adds an arm with the limit raised to 15 per unit, beyond any current the network can deliver. Clearing opens lines 10--13 and 13--14, and unit~32 stays connected through line 10--11. Table~\ref{tab:ilimarms} lists the peak currents and the limiter timing at each limit. Without a practical limit the unit-32 current peaks at 1.83 per unit, so the 2.0 per-unit default is never reached, even one line from the unit's point of interconnection. 

Every unit rides through in these five arms: post-fault active power returns to within 0.3\% of its pre-fault value and the frequency settles at 60.01~Hz. A sixth run at 1.0 per unit, a limit at rated current, does not recover (Figure~\ref{fig:ilimarms13}(e)).

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{narr_ilim_arms_bus13}
\caption{Bus-13 fault on the all-GFM fleet at five transient current limits, which opens lines 10--13 and 13--14. (a)~Peak of each unit's internal current magnitude on its own base. (b)~The same signal for the bus-32 unit through the fault. (c)~The first two and a half cycles after fault inception. (d)~RMS current of the bus-32 unit on the 230~kV side of its step-up transformer, in per unit. (e)~Voltage of the bus-32 unit on the 230~kV side of the generation-step-up transformer.}
\label{fig:ilimarms13}
\end{figure}

Figure~\ref{fig:peaki} compares the device-base peak currents, including a machine peak of 3.72 per unit at the corresponding bus. GFM current is shaped by the modeled current limit, whereas machine fault current depends strongly on sub-transient reactance. For the synchronous machine, there is no digital limiting logic to enforce a current limit.

Figure~\ref{fig:exec} makes the same peak current comparison for a separate fault at bus~39, on the 230~kV side of the largest unit's step-up transformer. Clearing opens lines 1--39 and 9--39 and leaves that unit islanded with the bus-39 load in both the all-synchronous-machine and the all-grid-forming cases. The machine's RMS current reaches 5.95 per unit of its 2000~MVA rating, against 1.25 per unit of 1667~MVA for the GFM unit at the 1.5 per-unit setting. Both units carry the islanded load after clearing, with active power within 0.02 per unit of its final value 0.6~s after fault inception; the machine's exciter returns the bus voltage to its pre-fault 1.03 per unit, whereas the GFM unit settles at 0.96 per unit.

The machine's offset fault current has no zero crossing on two phases for about 2.9~s, so the fault elements of the all-machine case were set to clear at the set time rather than at a current zero; the GFM fault cleared at current zeros within 7~ms of the set time. Different ratings, dispatch, and measurement locations remain relevant even when both traces are expressed per unit.

In the GFM runs, the active and reactive power traces immediately after clearing contain a 2--4~Hz settling component and a 60~Hz component below 10\% of the deviation, the latter from the power calculation during the transient; the machines that remain connected in the bus-10 study oscillate at the 0.85--1.0~Hz inter-machine mode, which is absent from the islanded machine of Figure~\ref{fig:exec}.
...
\caption{Voltage-dip and current-command signals in the original GFL implementation. This grid-following model is the only inverter model in the 39-bus studies whose circuit includes explicit semiconductor switch elements.}
...
A synchronous machine stores kinetic energy in its rotor, whereas an inverter generates its electrical phase through digital control code running on a microcontroller. The two phase states can occupy analogous positions in reduced equations without representing the same physical mechanism. An inverter draws energy from its DC source and circuit storage. 
...
In the analyzed runs the REGFM\_A1 controls are the $P$--$f$ droop of Equation~\eqref{eq:drooplaw}, whose frequency is integrated to the internal phase $\theta_{\mathrm{E}}$; the $Q$--$V$ droop with the voltage regulator, which regulates the point-of-interconnection voltage; the reactive-power limits; the overload controller, which acts only while filtered power is above $P_{\max}$ or below $P_{\min}$; and the transient current limit $I_{\max F}$. Table~\ref{tab:a1settings} lists the settings. In the 39-bus runs the unit powers stay below 0.76 per unit, so $P_{\max}$ is never reached. The plant-controller references $P_{\mathrm{ref}}$, $Q_{\mathrm{ref}}$, and $V_{\mathrm{ref}}$ are held at their initialized values. The model exposes no inertia constant (Figure~\ref{fig:pscada1params}); its equivalent inertia is the filtered-droop quantity $T_{\mathrm{Pf}}/2m_{\mathrm{p}}$ of Equation~\eqref{eq:heqdeq} only. No secondary control, that is, no slower loop that returns frequency and voltage to their reference values after the droop response (plant-level integral control or automatic generation control), acts in any run \cite{du2023a1}. The post-fault operating points are the droop equilibria, and the effect of such a loop on stability is not assessed here. Figures~\ref{fig:controlblocks} and~\ref{fig:gfmblocks} show the machine governor path and the inverter controls as configured.

\begin{table}[tbp]\centering\small
\caption{REGFM\_A1 settings in the analyzed runs, read from the PSCAD model files, in per unit on each unit's own base.}
\label{tab:a1settings}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.62\linewidth}}\toprule
Control & Settings\\\midrule
$P$--$f$ droop & $m_{\mathrm{p}}=0.01$; $T_{\mathrm{Pf}}=10$~ms; $P_{\mathrm{ref}}=0.6$ in the 39-bus cases, 0.50 in the single-device comparison, 0 in the inertia tests\\
$Q$--$V$ droop and voltage regulator & $m_{\mathrm{q}}=0.05$; $T_{\mathrm{Qf}}=T_{\mathrm{Vf}}=10$~ms; $k_{\mathrm{pv}}=0$; $k_{\mathrm{iv}}=5.86$~pu/s; $[E_{\min},E_{\max}]=[0,1.15]$; VFlag~$=1$\\
Reactive-power limits & $Q_{\max}=-Q_{\min}=0.44$; $k_{\mathrm{pqmax}}=3$; $k_{\mathrm{iqmax}}=20$~pu/s\\
Overload controller & $P_{\max}=0.9$; $P_{\min}=0$; $k_{\mathrm{ppmax}}=0.01$; $k_{\mathrm{ipmax}}=0.1$~pu/s\\
Fault current limiting & $I_{\max F}=2.0$ unless Table~\ref{tab:cases} states otherwise; coupling reactance $X_{\mathrm{L}}=0.15$\\
Plant controller & Disabled; $P_{\mathrm{ref}}$, $Q_{\mathrm{ref}}$, and $V_{\mathrm{ref}}$ held at their initialized values\\\bottomrule
\end{tabular}
\end{table}
...
\caption{Single-device response to a bolted three-phase-to-ground fault at the point of interconnection (0.001~$\Omega$, five cycles from 3.0~s, self-clearing). Top: terminal voltage. Bottom: current on each device's own rating, with the GFM transient-current limit of 2.0 per unit dashed. The shaded band is the fault.}
\label{fig:smibvi}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.85\textwidth]{narr_smib_sync_vs_gfm_Pf}
\caption{Single-device active-power and frequency responses at matched 0.50 per-unit dispatch.}
\label{fig:smibpf}
\end{figure}
...
\caption{Active and reactive current components for the single-device fault comparison, $I_{\mathrm{P}}=P/V$ and $I_{\mathrm{Q}}=Q/V$ on each device base.}
\label{fig:smibipiq}
\end{figure}
...
\label{eq:govpath}
\end{equation}
where $C(s)=K_{\mathrm{pgov}}+K_{\mathrm{igov}}/s$ is the governor PI controller ($K_{\mathrm{dgov}}=0$); $A(s)=1/(1+sT_{\mathrm{act}})$ is the actuator, a first-order lag; $T(s)=(1+sT_{\mathrm{c}})/(1+sT_{\mathrm{b}})$ is the turbine lead-lag; $F(s)=1/(1+sT_{\mathrm{pelec}})$ is the first-order lag of the electrical-power measurement selected by Rselect~$=1$; and $W_{\mathrm{f0}}=W_{\mathrm{fnl}}+P_{\mathrm{m0}}/K_{\mathrm{turb}}$ is the pre-disturbance fuel flow at dispatch $P_{\mathrm{m0}}$, which enters because Flag~$=1$ makes the fuel flow the product of valve stroke and speed. The power fed back through $F(s)$ is that of a machine swinging against a stiff network, $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}-2Hs\,\Delta\bar{\omega}$, which gives the factor $(1-2HR_{\mathrm{SG}}sF)$ with $H=5.46$~s; the factor is 1 at zero frequency and $0.47-0.09j$ at 1~Hz. In the 39-bus fleet $\Delta P_{\mathrm{e}}$ also carries the synchronizing power exchanged with the other machines, so Equation~\eqref{eq:govpath} is the single-machine form. 

At zero frequency the path gives $1/R_{\mathrm{SG}}=20$, so the five-to-one comparison $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ holds for these normalized damping coefficients in the steady-state limit only.

Figure~\ref{fig:damping} is an analytical estimate. It plots Equation~\eqref{eq:govpath}, evaluated from the governor and turbine constants of Table~\ref{tab:dyr_gov_params} alone, over the 0.1 to 3~Hz band of electromechanical swings for the fleet's dispatch range, $P_{\mathrm{m0}}=0.50$ to $1.00$ per unit of each machine's rating (bus~39 at 0.50, the other nine at 0.90--1.00). The shaded region marks where the machine's governor-and-turbine coefficient is negative, from its zero crossing at 0.38--0.41~Hz, depending on dispatch (Table~\ref{tab:dh}), to the 3~Hz upper end of that band. PSS2B and ESST4B are active in all machine cases, and no case without the stabilizer was simulated. The sign was confirmed in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency: the measured coefficient is negative from 0.5 to 2~Hz at both dispatches. The inverter's damping coefficient, by contrast, is flat where the machine's changes sign. No simulation result enters the figure or the analytical rows of Table~\ref{tab:dh}; both follow from the device parameters.
...
\caption{Analytical estimate of the equivalent damping versus perturbation frequency, normalized power per normalized frequency: the GFM filtered-droop coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}=100$ and the machine governor-and-turbine path of Equation~\eqref{eq:govpath}.}
\label{fig:damping}
\end{figure}
...
The inertia term depends on how quickly frequency changes, whereas the damping term depends on how far frequency has moved from nominal. A constant ramp therefore produces different time dependencies even though both contributions appear in the same active-power trace. Integrating that trace without separating the contributions from equivalent inertia versus equivalent damping assigns both to the inertia estimate. 
...
\caption{Contributions to the integrated power response during a constant rate-of-change-of-frequency ramp, from Equations~\eqref{eq:dpsplit} and~\eqref{eq:esplit}.}
\label{fig:esplit}
\end{figure}
...
\caption{Inertia estimate from Equation~\eqref{eq:hest} compared with the PSCAD integration-window sweep at $m_{\mathrm{p}}=0.1$: the line has intercept $T_{\mathrm{Pf}}/(2m_{\mathrm{p}})=0.05$~s, the equivalent inertia (dotted line), and slope $1/(4m_{\mathrm{p}})=2.5$. }
\label{fig:hestfit}
\end{figure}
...
\caption{PMView inertia estimate of the droop-controlled inverter versus short-circuit ratio. (a)~The estimate referred to the imposed $-1$~Hz/s ramp and referred to the inverter's internal droop frequency; the dotted line is the analytic $H_{\mathrm{eq}}+T/(4m_{\mathrm{p}})$. (b)~Average rate of change of the inverter's internal droop frequency over the 0.5~s test window.}
\label{fig:hestscr}
\end{figure}
...
We will align network topology, operating points, device ratings, bases, applicable control settings, and disturbance definitions across the positive-sequence and EMT models. The purpose of the next study phase is to understand which dynamics each modeling domain retains and how those dynamic differences affect engineering conclusions. The adapted PSS/E network and its E-TRAN translation provide the common starting point.
...
\subsubsection{Preliminary Application of Energy Functions for Transient Stability}
\label{sec:energymethod}
Energy-function methods offer a bridge from transient stability analysis of synchronous machines to GFM power systems \cite{sauer1998,pai1989,athay1979,bergen1981,chiang1988}. These methods compare an energy-like measure of the post-fault state with an energy boundary associated with unstable equilibria. Energy functions quantify the total energy of a system as the sum of kinetic and potential energies contributed from generators.  Applying energy functions to GFM systems requires attention to the assumptions behind the power-angle relationship and damping representation.

For the post-fault all-synchronous machine system with center-of-inertia states $(\theta,\tilde\omega)$, the transient energy function is the first integral of the swing equations \cite{sauer1998},
\begin{equation}
...
\label{eq:fi}
\end{equation}
with $P_{\mathrm{COI}}=\sum_i(P_{\mathrm{m},i}-P_{\mathrm{e},i})$ and $M_{\mathrm{T}}=\sum_i M_i$. Here $P_{\mathrm{m},i}$ is the mechanical power of a machine or the power reference $p^{*}$ of an inverter, and $G_{ij}$ and $B_{ij}$ are the conductance and susceptance of the reduced network between internal nodes $i$ and $j$ ($G_{ij}=\operatorname{Re}\{Y_{ij}\}$ and $B_{ij}=\operatorname{Im}\{Y_{ij}\}$, with $Y$ the admittance matrix of that reduced network); in the 39-bus cases the electrical power is taken from the constant-power network solution instead of a reduced admittance matrix, with the same dependence on the internal voltages and the coupling reactances. So $f_i$, and with it the potential energy $V_{\mathrm{PE}}$ and the shape of the surfaces in Figures~\ref{fig:esbelow} and~\ref{fig:esat}, contain the dispatch, the internal voltages, and the network admittances only. 

The inertia coefficients enter the kinetic term and the center-of-inertia weighting $M_i/M_{\mathrm{T}}$. Damping acts on how $V$ changes along a trajectory. $V$ is a Lyapunov function if it is zero at the post-fault equilibrium $(\theta^{\mathrm{s}},0)$, positive elsewhere in its neighborhood, and its derivative with respect to time along every trajectory, $\dot V$, is negative or zero; the equilibrium is then stable \cite{khalil2002}. 

The energy functions for machines use $D=0$, for which $\dot V=0$ along post-fault trajectories. For the GFM system, the damping $D_i=1/m_{\mathrm{p}}$ is retained in the swing equations: every unit shares the same power-filter time constant, so $D_i/M_i=1/T_{\mathrm{Pf}}$ is the same on every unit, the center-of-inertia transform remains exact, and $\dot V=-\sum_i D_i\tilde\omega_i^2\le 0$, so $V$ remains a Lyapunov function with the damping in place. For the single-machine model, $V_{\mathrm{cr}}=V_{\mathrm{PE}}(\delta_{\mathrm{u}})$, and the condition $V\le V_{\mathrm{cr}}$ at clearing is the equal-area criterion: the kinetic energy gained during the fault must not exceed the energy the post-fault system can still absorb before the unstable equilibrium \cite[Sec.~9.6.3]{sauer1998}.

For each synchronous machine and GFM system tested, the transient stability is quantified with the critical clearing time metric. Critical clearing time captures the maximum fault duration that a power system can withstand before losing a specified stability criterion. Stability is reflected in the behavior of the generators' rotor angles or the inverter angles with respect to time. 

The critical clearing time is calculated in three steps:
\begin{enumerate}
\item Compute the post-fault stable equilibrium $\theta^{\mathrm{s}}$ by solving $f_i(\theta)=0$ on the post-fault network. With the network intact after the fault clears, as in the cases here, $\theta^{\mathrm{s}}$ is the pre-fault operating point.
\item Obtain $V_{\mathrm{cr}}$ from the unstable equilibrium or from the maximum of $V_{\mathrm{PE}}$ along the sustained-fault trajectory.
\item Integrate the fault-on swing dynamic equations and record the time at which $V$ reaches $V_{\mathrm{cr}}$.
\end{enumerate}
In this report, each device is modeled as a constant voltage behind reactance, $E'$ behind $X'_{\mathrm{d}}$ for the machines and Equation~\eqref{eq:gfmswing} for the GFMs, with coefficients converted to the system base. The 39-bus network is retained with the model's constant-power loads, the fault is the bolted bus-14 fault with the network intact, and $V_{\mathrm{PE}}$ is evaluated as the path integral in Equation~\eqref{eq:tef} along the computed trajectory. Constant power is the most demanding of the static load types for stability \cite[Sec.~8.4.3]{sauer1998}, so this choice is conservative. 

For the constant-voltage, lossless single-machine model in Equation~\eqref{eq:swingeq}, the stable, $\delta_{\mathrm{s}}$, and unstable equilibria, $\delta_{\mathrm{u}}$, satisfy
\begin{equation}
...
We compare analytical estimates with simulated brackets $(a,b]$ formed by a stable fault-ride-through simulation for fault duration, $a$, and a loss of stability for the simulated fault duration, $b$. Any phase slip, even one that produces a stable post-fault equilibrium $360^\circ$ away from the original equilibrium, is considered unstable in our analyses.
...
\caption{Energy-method critical clearing times (navy) compared with PSCAD EMT brackets (orange), each bracket running from a successful to an unsuccessful clearing duration.}
\label{fig:efcct}
\end{figure}
...
\caption{Complementary views near the analytical critical clearing times: (a,b)~2D potential-energy contours and (c,d)~3D surfaces, synchronous machines on the left and GFM inverters on the right.}
\label{fig:esat}
\end{figure}

A likely reason is that the reduced model has no limiters. REGFM\_A1 limits its current, its internal voltage, and its active and reactive power (Figure~\ref{fig:gfmblocks}); once a limit is active, the power no longer follows the sinusoidal power-angle relation on which the energy function is built, and the dynamics become piecewise. Figure~\ref{fig:satblock} illustrates how saturation changes the power-angle characteristic and the associated unstable equilibrium. 

Future modeling studies will isolate the effects of these saturation blocks on critical clearing time prediction. Figure~\ref{fig:ilimitsweep} shows the measured single-inverter clearing-time bracket as the transient current limit is raised from 2.0 to 15 per unit with all other settings fixed: the bracket rises from $(0.925,0.931]$~s at 2.0 per unit to $(1.163,1.169]$~s at 12 per unit and is unchanged at 15 per unit, where the limit exceeds the unlimited fault-current demand.

\begin{figure}[tbp]\centering
\includegraphics[width=0.9\textwidth]{saturation_block}
\caption{Conceptual effect of current saturation on the power-angle dynamics during a fault.}
\label{fig:satblock}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.9\textwidth]{ilimit_sweep_emt}
\caption{Measured critical clearing time of the single inverter on the infinite bus versus the transient current limit $I_{\max F}$, all other settings fixed.}
\label{fig:ilimitsweep}
\end{figure}

\subsubsection{Modeling Goals}
\label{sec:nextsteps}
We will apply the simulation testbed we built over the past year to test and revise our analytical model for critical clearing time of an all-GFM system. This analytical model will provide relevant decision makers with the ``how'' of an accurate model and not just the ``what'' of simulated GFM power systems. Hardware or hardware-in-the-loop studies remain a future direction after the simulation comparisons establish appropriate test cases.
\begin{itemize}
\item \textbf{Limiter-aware stability estimates.} Develop a critical-clearing-time analytical model for single and multi-inverter systems to include saturation and determine conditions under which the estimate is conservative and accurate.
