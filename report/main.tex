...
\label{sec:gfmgap}
For the GFM system, the energy-function approach to quantifying transient stability overestimates the clearing margin for the tested GFM cases. In the 39-bus comparison, it predicts approximately 2.34~s against an EMT bracket of $(1.6,1.7]$~s. In the single-inverter infinite-bus comparison, it predicts 1.296~s against $(0.925,0.931]$~s.
...
The energy function approach may not work for the GFMs due to the presence of ``limiter'' saturation blocks throughout the GFM control parameters. These saturation blocks create piecewise fault-on dynamics for quantities like current and voltage. These piecewise dynamics are difficult to capture in energy functions. Figure~\ref{fig:satblock} illustrates how saturation changes the power-angle characteristic and the associated unstable equilibrium. 
