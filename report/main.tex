...
The damping coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and extracting the component of the mechanical power in phase with the speed deviation at $f_{\mathrm{m}}$. Each recorded signal $x(t)$ is fitted by least squares over the last 10~s of the record, trimmed to whole modulation cycles, as
\[
x(t)\approx c_0+c_1t+c_2t^2+c_3t^3+a_x\cos(2\pi f_{\mathrm{m}}t)+b_x\sin(2\pi f_{\mathrm{m}}t),\qquad \hat X=a_x-jb_x,
\]
where the cubic absorbs the slow drift and $\hat X$ is the complex amplitude of the response at $f_{\mathrm{m}}$. The coefficient follows from the swing equation, Equation~\eqref{eq:swingeq}, written for small deviations in terms of the per-unit speed deviation,
\[
