...
=\frac{K_{\mathrm{turb}}\,T(s)\,\bigl[A(s)\,C(s)-W_{\mathrm{f0}}\bigr]}{1+R_{\mathrm{SG}}\,F(s)\,K_{\mathrm{turb}}\,T(s)\,A(s)\,C(s)},
\label{eq:govpath}
\end{equation}
where $C(s)=K_{\mathrm{pgov}}+K_{\mathrm{igov}}/s$ is the governor controller ($K_{\mathrm{dgov}}=0$), $A(s)=1/(1+sT_{\mathrm{act}})$ the actuator, $T(s)=(1+sT_{\mathrm{c}})/(1+sT_{\mathrm{b}})$ the turbine lead-lag, $F(s)=1/(1+sT_{\mathrm{pelec}})$ the electrical-power feedback filter selected by Rselect~$=1$, and $W_{\mathrm{f0}}=W_{\mathrm{fnl}}+P_{\mathrm{m0}}/K_{\mathrm{turb}}$ the pre-disturbance fuel flow at dispatch $P_{\mathrm{m0}}$, which enters because Flag~$=1$ makes the fuel flow the product of valve stroke and speed. The feedback loop is closed with $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}$, which fixes the zero-frequency value $1/R_{\mathrm{SG}}=20$; above 0.3~Hz the feedback term has magnitude below 0.05 and does not affect the result. $D=0$ in the GENROU record. The five-to-one comparison is therefore $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ for these normalized coefficients in the zero-frequency limit. It is not a universal ratio of total system damping. The stabilizer contributes nothing to this steady-state comparison: PSS2B acts on its inputs through high-pass (``washout'') stages with time constants $T_{\mathrm{w1}}=T_{\mathrm{w2}}=T_{\mathrm{w3}}=2$~s in the Appendix~\ref{app:params} record, whose gain at zero frequency is zero \cite[Sec.~12.5, p.~770]{kundur1994}, so $1/R_{\mathrm{SG}}$ is the machine's net steady-state droop. At the 0.85--1.0~Hz mode the relevant machine quantities are instead the governor-path value of $-1.6$ to $-1.9$ and the net damping 1.8 in Table~\ref{tab:dh}.

Figure~\ref{fig:damping} plots Equation~\eqref{eq:govpath} against perturbation frequency for the fleet's dispatch range, $P_{\mathrm{m0}}=0.50$ to $1.00$ per unit of each machine's rating (bus~39 at 0.50, the other nine at 0.90--1.00). The coefficient starts at 20, falls as the actuator and turbine lags accumulate phase, crosses zero between 0.57 and 0.62~Hz, and reaches $-1.7$ to $-2.0$ at 1~Hz. The phase lag of the path has then exceeded $90^\circ$, so the mechanical-power response is in antiphase with the speed deviation, which is negative damping in the damping-torque sense of \cite[Sec.~2.1, Eq.~(2.2); Sec.~12.5]{kundur1994}: a torque component in phase with the speed deviation is damping, and a component in antiphase is negative damping. The sign change comes from the two lags in the path, $T_{\mathrm{act}}=0.5$~s and $T_{\mathrm{b}}=0.1$~s; a single first-order lag, the stand-in of \cite[Eq.~(51)]{chatterjee2026note}, never reaches $90^\circ$ and cannot change sign. Omitting the speed dependence of the fuel flow gives $-1.1$ at 1~Hz and a crossing at 0.703~Hz, so the sign does not depend on that term. Damper windings, excitation, the stabilizer, and network interactions also affect the total damping of a mode, and none of them is in Equation~\eqref{eq:govpath}.
...
In-phase coefficient, 1~Hz & 100 & $-1.7$ to $-2.0$ (governor)\\
Governor zero crossing & Not applicable & 0.57--0.62~Hz\\
...
\caption{Equivalent damping versus perturbation frequency. The filtered-droop coefficient is $D_{\mathrm{eq}}=100$; the machine governor-and-turbine path of Equation~\eqref{eq:govpath} starts at $1/R_{\mathrm{SG}}=20$ and its in-phase component changes sign at 0.57--0.62~Hz; the band spans the fleet dispatch of 0.50 to 1.00 per unit and the line is the median dispatch. Coefficients use normalized power per normalized frequency. The diamond is the net damping of the whole machine measured from the decay of the post-fault oscillation in the all-machine bus-14 case, 1.8 (1.4--2.2), at its 0.85--1.0~Hz mode.}
\label{fig:damping}
\end{figure}
