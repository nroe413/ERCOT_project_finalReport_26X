...
A synchronous machine stores kinetic energy in its rotor, whereas an inverter generates its electrical phase through digital control code running on a micro controller. The two phase states can occupy analogous positions in reduced equations without representing the same physical mechanism. An inverter draws energy from its DC source and circuit storage. 
...
\caption{GGOV1 gas turbine governor model of the machine, redrawn from Figure~3-5 of IEEE PES-TR1 \cite{pestr1} (original courtesy of GE) with its signal and parameter names. Table~\ref{tab:dyr_gov_params} gives the settings; in this study $K_{\mathrm{imw}}=K_{\mathrm{dgov}}=T_{\mathrm{eng}}=D_{\mathrm{m}}=0$, Rselect~$=1$, and Flag~$=1$.}
