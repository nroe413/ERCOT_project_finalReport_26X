...
\]
and the damping term is the power that opposes the speed deviation. A device whose power responds to speed therefore contributes $D=-\Delta P/\Delta\bar{\omega}$, positive when its power falls as the speed rises. For a sinusoidal speed swing only the part of the power response in step with the speed acts as damping; the part a quarter cycle out of step is in step with the acceleration and acts as inertia. The coefficient at a perturbation frequency $f$ is therefore the real part of the ratio of the complex amplitudes,
\begin{equation}
D(f)=-\operatorname{Re}\left\{\frac{\hat P_{\mathrm{mech}}}{\hat\omega}\right\}
=-\frac{|\hat P_{\mathrm{mech}}|}{|\hat\omega|}\cos(\varphi_{\mathrm{P}}-\varphi_{\omega}),
\label{eq:dmeas}
\end{equation}
where $\hat P_{\mathrm{mech}}$ and $\hat\omega$ are the complex amplitudes of the mechanical power and of $\Delta\bar{\omega}$, and $\varphi_{\mathrm{P}}$ and $\varphi_{\omega}$ their phases. This is the damping-torque coefficient of \cite{demello1969}, the component of the torque response in phase with the speed deviation, taken on the mechanical side.
...
\label{eq:govpath}
\end{equation}
where $C(s)=K_{\mathrm{pgov}}+K_{\mathrm{igov}}/s$ is the governor controller ($K_{\mathrm{dgov}}=0$), $A(s)=1/(1+sT_{\mathrm{act}})$ the actuator, $T(s)=(1+sT_{\mathrm{c}})/(1+sT_{\mathrm{b}})$ the turbine lead-lag, $F(s)=1/(1+sT_{\mathrm{pelec}})$ the electrical-power feedback filter selected by Rselect~$=1$, and $W_{\mathrm{f0}}=W_{\mathrm{fnl}}+P_{\mathrm{m0}}/K_{\mathrm{turb}}$ the pre-disturbance fuel flow at dispatch $P_{\mathrm{m0}}$, which enters because Flag~$=1$ makes the fuel flow the product of valve stroke and speed. The power fed back through $F(s)$ is that of a machine swinging against a stiff network, $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}-2Hs\,\Delta\bar{\omega}$, which gives the factor $(1-2HR_{\mathrm{SG}}sF)$ with $H=5.46$~s; the factor is 1 at zero frequency and $0.47-0.09j$ at 1~Hz, where it removes about half of the direct speed path. In the 39-bus fleet $\Delta P_{\mathrm{e}}$ also carries the synchronizing power exchanged with the other machines, so Equation~\eqref{eq:govpath} is the single-machine form. $D=0$ in the GENROU record.

At zero frequency the path gives $1/R_{\mathrm{SG}}=20$, so the five-to-one comparison $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ holds for these normalized coefficients in the steady-state limit only. The stabilizer does not enter it: PSS2B acts through washout stages ($T_{\mathrm{w1}}=T_{\mathrm{w2}}=T_{\mathrm{w3}}=2$~s in the Appendix~\ref{app:params} record) whose gain at zero frequency is zero \cite[Sec.~12.5, p.~770]{kundur1994}.

Figure~\ref{fig:damping} plots Equation~\eqref{eq:govpath} over the 0.1 to 3~Hz band of electromechanical swings for the fleet's dispatch range, $P_{\mathrm{m0}}=0.50$ to $1.00$ per unit of each machine's rating (bus~39 at 0.50, the other nine at 0.90--1.00). The governor holds speed by cutting fuel when the machine speeds up and adding fuel when it slows, so for a slow change its power change opposes the speed deviation and damps the swing; the actuator and the turbine delay that response. The coefficient starts at 20, falls as the two lags accumulate phase, crosses zero between 0.38 and 0.41~Hz, and reaches $-1.4$ to $-1.8$ at 1~Hz. Past the crossing the delay exceeds a quarter of the swing period, so the commanded power change arrives after the speed has reversed: mechanical power rises while the speed rises and feeds the swing. This is negative damping in the sense of \cite[Sec.~2.1, Eq.~(2.2); Sec.~12.5]{kundur1994}. The sign change needs both lags, $T_{\mathrm{act}}=0.5$~s and $T_{\mathrm{b}}=0.1$~s; a single first-order lag, the stand-in of \cite[Eq.~(51)]{chatterjee2026note}, never reaches $90^\circ$ and cannot change sign. Closing the feedback with $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}$ instead, or omitting the speed dependence of the fuel flow, moves the crossing to 0.6--0.7~Hz and the 1~Hz value to between $-1.1$ and $-2.0$ without changing the sign. Damper windings, excitation, the stabilizer, and network interactions also damp a mode, and none of them is in Equation~\eqref{eq:govpath}.

The coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and evaluating Equation~\eqref{eq:dmeas} at $f_{\mathrm{m}}$. Because the records also carry a slow drift from the governor's integrator and some ripple, the amplitudes come from a lock-in fit: each signal $x(t)$ is fitted by least squares \cite{ieee1057} over the last 10~s of the record, trimmed to whole modulation cycles, as
\[
x(t)\approx c_0+c_1t+c_2t^2+c_3t^3+a_x\cos(2\pi f_{\mathrm{m}}t)+b_x\sin(2\pi f_{\mathrm{m}}t),\qquad \hat X=a_x-jb_x,
\]
where the cubic absorbs the drift, and the two signals share a time axis so that their phases can be subtracted. The machine's mechanical power follows from the recorded torque, $\hat P_{\mathrm{mech}}=\hat T_{\mathrm{mech}}+T_{\mathrm{m0}}\hat\omega$ in per unit with $T_{\mathrm{m0}}$ the pre-modulation torque; for the inverter the terminal power and the internal droop frequency take the places of $\hat P_{\mathrm{mech}}$ and $\hat\omega$.

With the governor deadband removed from a copy of the record, the measured coefficient follows Equation~\eqref{eq:govpath} within 0.01 at 0.50 per unit ($+2.29$, $-0.86$, $-1.43$, $-1.42$, $-1.10$, and $-0.82$ at 0.3, 0.5, 0.7, 1.0, 1.5, and 2.0~Hz) and within 0.1 at 0.95 per unit, with the zero crossing between 0.3 and 0.5~Hz; these are the measured points in Figure~\ref{fig:damping}. With the supplied deadband of 0.0002 per unit the magnitude is 0.2 to 0.3 smaller at the test's 0.0015 per-unit speed swing, because the governor ignores the part of the error inside the deadband; at disturbance amplitudes the deadband is not a factor. The same test on the single-inverter case (dispatch 0.50 and 0.60 per unit) gives 99.9 at 0.3~Hz falling to 97.0 at 2~Hz; the decrease is the 0.02~s smoothing of the power meter against the 0.01~s power-filter lead, $\mathrm{Re}[(1+j\omega T_{\mathrm{Pf}})/(1+0.02j\omega)]$, which predicts 99.2 at 1~Hz and 97.0 at 2~Hz, while the filtered droop power reproduces $1/m_{\mathrm{p}}=100$ at every frequency. The inverter's coefficient is flat where the machine's changes sign.

The net machine damping in Table~\ref{tab:dh} comes from the all-machine bus-14 case (30~s record, Figure~\ref{fig:allsync}). After clearing, the ten generator-bus active powers oscillate together at the 0.85--1.0~Hz inter-machine mode and decay; a Fourier transform of the 4--30~s record gives the frequency, and the logarithm of the ratio of successive peaks gives the decay rate $\sigma=0.06$--$0.10$~s$^{-1}$ for the eight machines away from the fault (buses 31 and 32 carry a faster local component and are excluded). With the electrical power responding to the angle through the synchronizing coefficient, $\Delta P_{\mathrm{e}}=K_{\mathrm{S}}\Delta\delta$ and $\Delta\dot\delta=\omega_0\Delta\bar{\omega}$, the swing equation above becomes
\[
...
\]
the damped oscillator $\ddot x+2\sigma\dot x+\omega_{\mathrm{n}}^2x=0$ with $\sigma=D/4H$ \cite[Sec.~12.3]{kundur1994}. The measured decay gives $D_{\mathrm{net}}=4H\sigma\approx1.8$ (1.4--2.2) on the machine base with $H=5.46$~s. The value is net: the governor path, the damper windings, the exciter and stabilizer, and the network interactions all contribute to the decay, and their shares are not separated. The relation is exact for one machine against a stiff system with constant $D$; for the inter-machine mode it gives a per-machine equivalent. PSS2B and ESST4B are active in all machine cases; no case without the stabilizer was simulated.

The inertia comparison runs the other way: $T_{\mathrm{Pf}}=10$~ms and $m_{\mathrm{p}}=0.01$ give $H_{\mathrm{eq}}=0.5$~s, roughly one eleventh of the machine's 5.46~s. Table~\ref{tab:dh} collects the values, keeping the governor contribution separate from the net damping. Strong equivalent damping with low equivalent inertia is consistent with the faster GFM settling.
