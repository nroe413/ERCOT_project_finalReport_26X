...
An L-C-L (LCL) filter places two inductors and a shunt capacitor between the converter and the grid. The controller regulates current through the converter-side inductor and voltage across the filter capacitor, while outer controls govern power and voltage behavior. Different control approaches use this same physical interface differently. This report focuses on grid-forming (GFM) control and compares its response characteristics to those of conventional synchronous machines.
...
\caption{All-GFM RMS current responses to the bus-14 reference fault, in kA. The inverter transient-current setting is $I_{\mathrm{maxF}}=2.0$ per unit on each device base.}
\label{fig:allgfmi}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{allgfm_p}
\caption{All-GFM active-power responses to the bus-14 reference fault, in MW. Signed powers follow the measurement-channel direction.}
\label{fig:allgfmp}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{allgfm_q}
\caption{All-GFM reactive-power responses to the bus-14 reference fault, in MVAr.}
\label{fig:allgfmq}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig2_allSYNC_3PG_2x2}
\caption{All-machine response to the bus-14 reference fault: RMS voltage and current, active power, and reactive power. The generator-bus active powers oscillate at the 0.85--1.0~Hz inter-machine mode with decay rates of 0.06--0.10~s$^{-1}$. The legend lists the monitored buses; bus~14 is isolated by clearing.}
\label{fig:allsync}
\end{figure}
...
\caption{Bus-10 current-limit study with a five-cycle fault and no reclosing. With a 1.5 per-unit setting, bus~32 reaches approximately 1.70 per unit before falling to zero; nine units continue carrying current.}
\label{fig:peakits}
\end{figure}
...
\caption{Bus-14 reference fault on the all-GFM fleet at three transient current limits, 10~s records with identical dispatch, fault, and clearing. Left: peak of each unit's internal current magnitude on its own base, with the three limit lines. Right: the same signal for the bus-32 unit through the fault.}
\label{fig:ilimarms}
\end{figure}
...
\caption{Peak currents for the bus-10 disturbance, normalized on individual device bases. The GFM transient-current setting is 1.5 per unit.}
\label{fig:peaki}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{exec_gfm_vs_sync_bus39_pu}
\caption{Bus-39 terminal responses in the bus-10 study: all-machine (blue) and all-GFM (red), each on its own device base. The GFM case shows smaller fault current and faster settling; the two cases have different pre-fault power injections.}
\label{fig:exec}
\end{figure}
...
\caption{Unbalanced faults at bus~14, all-machine and all-GFM fleets, 10~s records with the reference timing and clearing. Left: bus-16 RMS voltage through the fault (SLG solid, LLG dashed). Right: peak RMS current of each unit on its own base.}
\label{fig:unbal}
\end{figure}
...
\caption{Single-device fault response: terminal voltage and current for the synchronous machine and GFM inverter. The GFM transient-current setting is 2.0 per unit; both devices are dispatched at 0.50 per unit.}
\label{fig:smibvi}
\end{figure}
