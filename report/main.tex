...
\caption{Bus-13 fault on the all-GFM fleet at four transient current limits, 10~s records with identical dispatch, fault, and clearing; clearing opens lines 10--13 and 13--14. (a)~Peak of each unit's internal current magnitude (the inverter's own measurement at its terminals, 10~kV side of the step-up transformer) on its own base, with the three limit lines; the 15 per-unit arm has no practical limit. (b)~The same signal for the bus-32 unit through the fault. (c)~Reaction time of the current limiter over the first two and a half cycles after fault inception; for each setpoint the legend gives when the current first exceeds the limit (open circle) and when it is back within 2\% of the limit and held there for a cycle (filled circle). Instantaneous failures of the semiconductor switches are not modeled.}
