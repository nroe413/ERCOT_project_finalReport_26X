...
The inertia term depends on how quickly frequency changes, whereas the damping term depends on how far frequency has moved from nominal. A constant ramp therefore produces different time dependencies even though both contributions appear in the same active-power trace. Integrating that trace without separating the contributions assigns both to the inertia estimate. The distinction is between interpreting the test result and changing the configured device inertia, not between a resource with energy and one without energy.
