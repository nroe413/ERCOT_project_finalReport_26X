...
We developed a reusable simulation testbed to study power systems with a high share of grid-forming (GFM) inverters. The University of Texas at Austin collaboration with the Electric Reliability Council of Texas (ERCOT) began with an IEEE 39-bus PSS/E network and translated it through E-TRAN into PSCAD. We equipped the adapted benchmark with representative machine dynamics supplied by ERCOT and with inverter models for replacement studies. The first-year work primarily concerns electromagnetic transient (EMT) simulations in PSCAD; the PSS/E starting case provides the network foundation for corresponding positive-sequence studies in the next phase.

Battery storage motivates this work because it can absorb energy when supply is abundant and inject energy when needed. Variable renewable generation and rapidly changing loads increase the importance of this flexibility. ERCOT's advanced grid-support requirements for new inverter-based storage resources also make GFM behavior increasingly relevant to system studies. Understanding how these controls behave in steady state, during faults, and in the post-fault period is therefore important.

The testbed combines configurable network disturbances, resource models, and scripts that process the simulation outputs. We developed eleven configurations by replacing zero through ten synchronous machines with the selected droop-controlled GFM model. Because generator ratings and operating points differ, we identify intermediate configurations by replacement count rather than assigning each unit an equal percentage of generation. The same network framework supports later studies of other GFM controls, grid-following (GFL) resources, mixed generation, and load disturbances.

The reference simulations show recovery of the connected network in configurations spanning the all-machine and all-GFM endpoints. The selected GFM responses settle faster than the corresponding machine responses after the three-phase fault. Inverter fault currents also remain much closer to the configured current limit than the larger machine currents. These observations concern the disturbances and resource configurations studied, and continued network recovery is distinct from continued output by a resource disconnected during clearing.

The analytical comparison connects these observations to familiar machine dynamics. A synchronous machine has a physical rotor whose motion determines its electrical phase, whereas a GFM controller generates an electrical phase without a rotating shaft. Under unsaturated filtered droop, the two models can be written in the same reduced swing-equation form. The GFM droop gain sets an equivalent damping coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}$, while the power-measurement filter and droop gain set equivalent inertia. For the selected parameters, the low-frequency damping coefficient is five times the machine governor droop coefficient on the corresponding normalized bases.

This equivalent damping provides an important interpretation of the strongly damped GFM responses, but it is not the only control property that affects fault recovery. Current and voltage limits change the power that an inverter can deliver during severe disturbances. The inertia estimation tests further show that damping-related power contributes to the estimated inertia over the test window. The preliminary energy-method investigation examines how those reduced dynamics compare with EMT fault-duration limits.

The next phase will align topology, operating points, ratings, bases, applicable controls, and disturbances in corresponding PSS/E and PSCAD studies. The objective is to understand differences between modeling domains, not to force identical trajectories. We will also extend the preliminary analytical work to investigate how current limits, voltage limits, and droop settings influence transient stability. Together, the testbed and analytical interpretation provide a foundation for guidance to manufacturers and power-system engineers.
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=2pt, align=center, inner sep=5pt,
              line width=0.8pt},
  stage/.style={box, draw=utcharcoal, fill=utcharcoal!6, text width=42mm},
  side/.style={box, draw=utorange, fill=utorange!7, text width=52mm},
  proc/.style={box, draw=utblue, fill=utblue!6, text width=42mm},
  arr/.style={-{Stealth[length=2.6mm]}, line width=0.9pt,
              color=utcharcoal}
]
\node[stage] (psse) {\textbf{PSS/E case}\\[1pt]
  {\footnotesize IEEE 39-bus, 10 machines}};
\node[stage, right=10mm of psse] (etran) {\textbf{E-TRAN}\\[1pt]
  {\footnotesize $\to$ EMT network}};
\node[stage, right=10mm of etran] (pscad) {\textbf{PSCAD EMT}\\[1pt]
  {\footnotesize fault studies}};
\node[side, below=7mm of psse] (dyr)
  {ERCOT \texttt{.dyr} machine dynamics\\[1pt]
  {\footnotesize (Table~\ref{tab:dyr_gov_params},
   Appendix~\ref{app:params})}};
\node[side, text width=42mm, below=7mm of pscad] (pnnl)
  {PNNL REGFM\_A1 library\\[1pt]
  {\footnotesize \cite{du2023a1,pnnlgithub}}};
\node[proc, below=7mm of dyr] (figs)
  {\textbf{Report figures}\\[1pt]
  {\footnotesize (Appendix~\ref{app:plotting})}};
\node[proc] (proc) at (etran |- figs)
  {\textbf{Python pipeline}};
\node[proc] (out) at (pscad |- figs)
  {\textbf{EMTDC output}\\[1pt]
  {\footnotesize \texttt{.out} + \texttt{.inf}}};
\draw[arr] (psse) -- (etran);
\draw[arr] (etran) -- (pscad);
\draw[arr, color=utorange] (dyr) -- (psse);
\draw[arr, color=utorange] (pnnl) -- (pscad);
\draw[arr, color=utblue] (pscad.east) -- ++(2.5mm,0) |- (out.east);
\draw[arr, color=utblue] (out) -- (proc);
\draw[arr, color=utblue] (proc) -- (figs);
\end{tikzpicture}
\caption{Testbed construction and output processing. E-TRAN converts the IEEE 39-bus PSS/E case with representative machine dynamics to PSCAD; REGFM\_A1 units replace machines in selected configurations. The numerical-output workflow converts EMTDC channels into tables for analysis and plotting.}
\label{fig:testbedflow}
\end{figure}

Table~\ref{tab:dyr_gov_params} lists the governor and turbine settings used by the machine model. Appendix~\ref{app:params} gives the machine, excitation, governor, and stabilizer record in PSS/E format. These dynamic settings act alongside the network operating point to determine the machine response. The parameter units follow the corresponding normalized control signals \cite{ggovmanual}.
...
\caption{GGOV1 governor and turbine parameters for the synchronous-machine reference \cite{pestr1,ggovmanual}.}
...
\caption{GGOV1 governor and turbine path of the machine model as configured, with the Table~\ref{tab:dyr_gov_params} settings. The speed error $\omega_{\mathrm{ref}}-\omega$ less the droop feedback $R\,P_{\mathrm{e}}$ (Rselect~$=1$, $P_{\mathrm{e}}$ filtered by $T_{\mathrm{pelec}}$) is clipped to [minerr, maxerr] and drives the proportional, integral, and derivative branches; the low-value gate passes the smallest of the governor, load-limiter, and acceleration-limiter demands to the actuator, whose stroke is rate- and position-limited; with Flag~$=1$ the fuel flow is the valve stroke scaled by speed, and the turbine power is $K_{\mathrm{turb}}(W_{\mathrm{f}}-W_{\mathrm{fnl}})$ through the lead-lag block. The power controller is disabled ($K_{\mathrm{imw}}=0$) and $T_{\mathrm{eng}}=0$. The integral term is $K_{\mathrm{igov}}$.}
...
\caption{Single-device active-power and frequency responses at matched 0.50 per-unit dispatch. The selected GFM model settles faster than the machine case.}
\label{fig:smibpf}
\end{figure}
...
For the GGOV1 calculation, the in-phase damping contribution is $D+\operatorname{Re}\{G(j\Omega)\}/R_{\mathrm{SG}}$, where $G$ is the normalized governor-path response, $R_{\mathrm{SG}}$ is permanent droop, and $\Omega$ is perturbation angular frequency. With the study parameters in Table~\ref{tab:dyr_gov_params}, the zero-frequency value is $1/R_{\mathrm{SG}}=20$. The five-to-one comparison is therefore $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ for these normalized coefficients in the zero-frequency limit. It is not a universal ratio of total system damping. The stabilizer contributes nothing to this steady-state comparison: PSS2B acts through washout stages ($T_{\mathrm{w1}}=T_{\mathrm{w2}}=T_{\mathrm{w3}}=2$~s in the Appendix~\ref{app:params} record), whose steady-state gain is zero \cite{kundur1994}, so $1/R_{\mathrm{SG}}$ is the machine's net steady-state droop. At the 0.85--1.0~Hz mode the relevant machine quantities are instead the governor-path value $-1.1$ and the ringdown net damping 1.8 in Table~\ref{tab:dh}.

Figure~\ref{fig:damping} shows the machine governor's in-phase contribution crossing zero at 0.703~Hz and reaching $-1.1$ at 1~Hz. Its phase lag has then exceeded $90^\circ$, so that contribution no longer opposes speed deviation, in the damping-torque sense of \cite{kundur1994}. Damper windings, excitation, the stabilizer, and network interactions also affect total modal damping. The net machine damping in Table~\ref{tab:dh} is inferred from the all-machine bus-14 case: the generator-bus active powers ring down at 0.85--1.0~Hz with decay rates $\sigma=0.06$--$0.10$~s$^{-1}$, from the log decrement of successive peaks over 4--30~s, giving $D_{\mathrm{net}}=4H\sigma\approx1.8$ (1.4--2.2) on the machine base with $H=5.46$~s. The excess of this value over the governor path is not attributed to individual components. PSS2B and ESST4B are active in all machine cases; no case without the stabilizer was simulated. The positive GFM coefficient provides an important interpretation of the less oscillatory response, without attributing every transient difference to that coefficient alone.

The inertia comparison runs in the opposite direction. The study values $T_{\mathrm{Pf}}=10$~ms and $m_{\mathrm{p}}=0.01$ give $H_{\mathrm{eq}}=0.5$~s, roughly one eleventh of the machine's 5.46~s. Table~\ref{tab:dh} summarizes the comparison, keeping the governor contribution separate from the machine ringdown estimate. Strong equivalent damping together with low equivalent inertia is consistent with the observed faster GFM settling.

\begin{table}[tbp]\centering
\caption{Selected inertia and damping quantities. Damping coefficients use normalized power per normalized frequency; the machine ringdown estimate represents the complete response rather than the governor alone.}
\label{tab:dh}
\small
...
In-phase coefficient, 1~Hz & 100 & $-1.1$ (governor)\\
Governor zero crossing & Not applicable & 0.703~Hz\\
Damping inferred from ringdown & --- & 1.8 (1.4--2.2)\\
...
\caption{Equivalent damping versus perturbation frequency. The filtered-droop coefficient is $D_{\mathrm{eq}}=100$; the machine governor contribution starts at $1/R_{\mathrm{SG}}=20$ and its in-phase component changes sign at 0.703~Hz. Coefficients use normalized power per normalized frequency. The diamond is the net damping inferred from the all-machine ringdown, 1.8 (1.4--2.2), at the measured 0.85--1.0~Hz mode.}
\label{fig:damping}
\end{figure}
...
\bibitem{ggovmanual} PowerWorld Corporation, \emph{Governor Model: GGOV1 and GGOV1D}, parameter descriptions and block diagram. \url{https://www.powerworld.com/WebHelp/Content/TransientModels_HTML/Governor\%20GGOV1\%20and\%20GGOV1D.htm}
\end{thebibliography}
