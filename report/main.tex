...
\input{figures/energy_surface_style.tex}
...
We developed a reusable simulation testbed to study power systems with a high share of generation provided by grid-forming (GFM) inverters. The University of Texas at Austin collaboration with the Electric Reliability Council of Texas (ERCOT) began with an IEEE 39-bus PSS/E network, translated to PSCAD through the E-TRAN software package. We equipped it with machine dynamics supplied by ERCOT and inverter models from Pacific Northwest National Lab (PNNL) for machine replacement studies. The first-year work primarily concerns electromagnetic transient (EMT) simulations in PSCAD; the PSS/E starting case provides the network foundation for corresponding positive-sequence studies in the next phase of the collaboration.

Battery storage motivates this work. Batteries absorb energy when supply is abundant and inject it on short notice. Variable renewable generation and rapidly changing loads raise the value of that flexibility. ERCOT's advanced grid-support requirements for new inverter-based storage resources also make GFM behavior increasingly relevant to system studies. Understanding how these controls behave in steady state, during faults, and in the post-fault period is therefore important.

The testbed combines configurable network disturbances, resource models, and scripts that process the simulation outputs. Eleven configurations replace zero through ten synchronous machines with the selected droop-controlled GFM model; the testbed also supports later studies of other GFM controls, grid-following (GFL) resources, mixed generation, user-defined load profiles, and load disturbances. The results are specific to this model and the tested cases: one droop-controlled GFM implementation and a limited set of disturbances at two buses within the 39 bus network. Other GFM controls, other settings of this one, or other scenarios may behave differently.

The reference simulations show fault-ride-through recovery of the connected network in generation configuration from all-machine systems to all-GFM systems. The selected GFM responses settle faster than the corresponding machine responses after the three-phase to ground, "bolted" fault. Inverter fault currents also remain much closer to the configured current limit than the larger machine currents. These observations concern the disturbances and resource configurations studied, and continued network recovery is distinct from continued output by a resource disconnected during clearing.

The analytical comparison connects these observations to familiar machine dynamics. A synchronous machine's rotor motion sets its electrical phase; a GFM controller generates that phase without a rotating shaft. Under unsaturated filtered droop, the two models can be written in the same reduced swing-equation form. The GFM droop gain sets an equivalent damping coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}$, while the power-measurement filter and droop gain set equivalent inertia. For the selected parameters, the low-frequency damping coefficient is five times the machine governor droop coefficient on the corresponding normalized bases.

This equivalent damping explains the strongly damped GFM responses, but it is not the only control property that affects fault recovery. Current and voltage limits change the power that an inverter can deliver during severe disturbances. The inertia estimation tests further show that damping-related power contributes to the estimated inertia over the test window. The preliminary transient stability energy-method investigation examines how grid-reduced dynamics compare with EMT fault-duration limits.

The next phase will align topology, operating points, ratings, bases, applicable controls, and disturbances in corresponding PSS/E and PSCAD studies. The objective is to understand differences between modeling domains. We will also extend the preliminary analytical work to investigate how current limits, voltage limits, and droop settings influence transient stability. Together they provide a foundation for guidance to manufacturers and power system engineers.

\clearpage
\section{Introduction}

This report presents the first-year PSCAD electromagnetic transient (EMT) testbed and its initial application to machine and inverter disturbance studies. It follows the development of the network, the simulated responses, and the reduced dynamic models used to interpret them. The adapted IEEE 39-bus system provides a common setting for this comparison, while its PSS/E origin supports the next phase of corresponding positive-sequence studies.
...
A battery connects to a three-phase AC network through a DC--AC inverter, as Figure~\ref{fig:invsymbol} illustrates. Semiconductor switches and passive capacitors and inductors shape the AC output from the DC source. A micro-controller changes each switch's duty cycle, the fraction of a switching period spent on, using measured voltage and current. The resulting commands regulate the electrical variables needed to deliver the selected power output.

An LCL filter places two inductors and a shunt capacitor between the converter and the grid. The controller regulates current through the converter-side inductor and voltage across the filter capacitor, while outer controls govern power and voltage behavior. Different control approaches use this same physical interface differently. This report focuses on grid-forming (GFM) control and compares its response characteristics to those of conventional synchronous machines.

\begin{figure}[b]\centering
\input{figures/tikz_inverter_overview.tex}
...
\caption{ERCOT battery energy storage capacity by commercial-operation year, alongside 36.2~GW in the interconnection queue with executed agreements. Sources: July 2026 Monthly Outlook for Resource Adequacy and Generator Interconnection Status reports \cite{mora2026,gis2026}.}
\label{fig:ercotbess}
\end{figure}

\label{sec:gflgfm}

Conventional grid-following (GFL) control uses an externally established voltage waveform as its synchronization reference, commonly through a phase-locked loop. Within normal voltage limits, its output current phasor remains approximately unchanged immediately after a disturbance, before outer controls adjust the injected power \cite{unifi2026}. A solver's use of a current-source equivalent does not determine this classification: a GFM voltage source behind impedance can also be converted to a Norton equivalent current-source representation. The distinction concerns control behavior and synchronization, not simply the circuit element used in a numerical network solution.

Three terms connect GFM controls to the machine-based interpretation developed in this report. Their meanings depend on the reduced model and its operating assumptions. In particular, an analogy between equations does not make the physical sources identical.
\begin{enumerate}
\item The \emph{inverter angle} is a controller-generated electrical phase. Like a machine's electrical rotor angle, its displacement relative to other sources influences power transfer. It is not the position of a physical shaft.
\item The \emph{equivalent inertia} of filtered droop arises from the lag in its power measurement and depends on the filter time constant and droop gain \cite{chatterjee2026note}. Shortening the filter reduces this coefficient without changing a physical moment of inertia. The electrical energy still comes from the DC source and circuit storage.
\item The \emph{equivalent damping} relates normalized frequency deviation to normalized power response. The droop gain sets this coefficient in the reduced GFM model. Section~\ref{sec:dampinertia} distinguishes it from a machine governor's frequency-dependent contribution and from damping inferred from a complete system response.
\end{enumerate}
...
The adapted IEEE 39-bus network contains ten generator buses \cite{pscad39bus,pscad39note}. Its EMT representation includes Bergeron transmission lines, generator step-up transformers, and constant-power loads (E-TRAN load model type~0: fixed real and reactive power) \cite{pscadbergeron}. E-TRAN translates the network parameters and initial load-flow operating point from PSS/E into PSCAD \cite{etran}. The load representation matters for the stability estimates of Section~\ref{sec:energymethod}: with these constant-power loads the ten-machine estimate is 0.248~s, inside the EMT bracket, whereas a constant-impedance representation of the same loads gives 0.479~s. The resulting benchmark uses a 230~kV transmission network with representative ERCOT-supplied machine dynamics, rather than representing actual Texas transmission facilities.

The ten machines share GENROU, ESST4B, GGOV1, and PSS2B dynamic parameters, including inertia $H=5.46$~s. Sharing these parameters does not imply identical ratings or dispatch. Figure~\ref{fig:testbedflow} summarizes the construction and processing workflow. Figures~\ref{fig:faultlogic} and~\ref{fig:faultcontrol} show the controls used to select fault location, type, timing, and associated breaker action.

The fault controls combine a central configuration panel with logic attached to individual transmission lines. We specify the fault start time, location, type, and duration centrally, while the line controls implement the corresponding fault and breaker commands. The protection settings distinguish removal of the fault from restoration of the connection. This arrangement allows the same network to support either a reclosed connection or a post-fault topology in which the selected connection remains open.
...
\caption{Central fault-control settings for the bus-14 reference disturbance: start time 3.0~s, type code 7, and duration 0.0833~s, approximately five cycles at 60~Hz.}
...
The instrumentation records per-bus instantaneous and root-mean-square (RMS) voltage and current, active and reactive powers, and frequency quantities. Frequency analysis includes the nadir, recovery, maximum deviation, and rate of change of frequency. We parse the PSCAD output files and normalize device quantities on their apparent-power bases for plotting. Physical-unit network plots and device-normalized plots retain their respective measurement locations and voltage levels.

Voltage and current interpretation depends on the selected bases and on whether the signal is instantaneous or RMS. We distinguish line-to-line voltage from line-to-neutral voltage when interpreting the three-phase quantities. Device-base normalization makes the controller settings readable but does not, by itself, make unequal ratings or operating points identical. Appendix~\ref{app:plotting} describes the channel-processing conventions.
...
\node[font=\footnotesize, anchor=west, inner sep=1pt]
  at ([xshift=1.5mm, yshift=-2.5mm]pscad.south) {dropped in};
\end{tikzpicture}
\caption{Testbed workflow. E-TRAN converts the IEEE 39-bus PSS/E case into a PSCAD EMT network; synchronous-machine dynamics from the ERCOT-supplied record and grid-forming inverter dynamics from the PNNL REGFM\_A1 model are dropped into that network; Python test scripts set up and run the simulations, and processing scripts convert the EMTDC output into tables, metrics, and the report's figures. The interpretation of those results is not automated.}
...
external reference inputs and the internal-signal plot taps.}
...
Four models were evaluated: REGFM\_A1, REGFM\_B1, the NLR GFM model, and the PSCAD library GFM model. REGFM\_A1 was selected for the system studies as the configuration usable as shipped; the other models were not tuned further. Appendix~\ref{app:altmodels} records the alternative-model diagnostics.
...
The machine example is the 650~MVA Gen-32 test case. Its small-step test raises voltage from 1.00 to 1.03 per unit at 3~s and produces a slower response than the selected REGFM\_A1 comparison. Figure~\ref{fig:smibgfm} additionally records the inverter-side LCL quantities during the REGFM\_A1 ride-through test. Voltage, current, active power, reactive power, and droop frequency together describe how the device supplies the grid through the disturbance.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig3_mqt_legacy_lvrt}
\caption{REGFM\_A1 response to the legacy low-voltage ride-through profile in PMView.}
\label{fig:mqt}
\end{figure}
...
\caption{Single REGFM\_A1 fault response with a 2.0 per-unit transient-current limit: normalized voltage, current, active and reactive powers, and frequency in Hz.}
\label{fig:smibgfm}
\end{figure}

\subsection{Study Design}
\label{sec:studydesign}
We first applied a three-phase-to-ground fault at bus~14 to the all-machine system. The fault resistance is 0.01~$\Omega$, a low-impedance approximation to a bolted fault. Initial machine studies considered both restoration of the faulted connection and clearing with the connection left open. The reference case used for the replacement progression has a five-cycle fault without reclosing.

We selected a short reference disturbance to represent a practical protection timescale. The 0.0833~s duration corresponds to approximately five cycles at 60~Hz and is applied from 3.0~s. After establishing the machine baseline, we replaced machines one at a time and reapplied the same reference fault. Extended-duration tests discussed later explore transient stability limits rather than recommended protection operating times. Table~\ref{tab:cases} lists the simulated cases. Network faults were applied at bus~14 and bus~10 only; other fault locations were not tested. All faults in the replacement progression and the current-limit studies are three-phase-to-ground; single-line-to-ground and double-line-to-ground checks at bus~14 are reported in Section~\ref{sec:unbal}. No overcurrent trip is modeled in the replacement progression or the current-limit studies; the hardware-trip check of Section~\ref{sec:ilim} adds one to the limited units.
...
Current-limit sensitivity (Section~\ref{sec:emt}) & 10 GFM units & 14 & 5 cycles; lines to bus~14 opened, no reclosing & 2.0; 1.5; 1.2\\
Hardware-trip check (Section~\ref{sec:ilim}) & 10 GFM units, trip on units 32 (bus-10 fault) and 30--32 (bus-14 fault) & 10; 14 & 5 cycles; lines opened, no reclosing; trip at 2.0 (bus 10), 1.5 and 1.2 (bus 14) times rated peak current, 1~ms dwell & 1.5; 1.2\\
...
\caption{Bus-voltage responses with one replaced machine (top) and two replaced machines (bottom) under the bus-14 reference fault. Bus~14 is isolated by clearing; its trace is the residual voltage of the de-energized bus.}
\label{fig:vrms12gfm}
\end{figure}
...
\caption{All-GFM bus-voltage responses to the bus-14 reference fault. RMS voltages are shown in kV for the monitored network locations.}
\label{fig:allgfm}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{allgfm_irms}
\caption{All-GFM RMS current responses to the bus-14 reference fault, in kA, measured on the 230~kV side of each unit's step-up transformer. The inverter transient-current setting is $I_{\mathrm{maxF}}=2.0$ per unit on each device base.}
\label{fig:allgfmi}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{allgfm_p}
\caption{All-GFM active-power responses to the bus-14 reference fault, in MW, measured on the 230~kV side of each unit's step-up transformer. Signed powers follow the measurement-channel direction.}
\label{fig:allgfmp}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{allgfm_q}
\caption{All-GFM reactive-power responses to the bus-14 reference fault, in MVAr, measured on the 230~kV side of each unit's step-up transformer.}
\label{fig:allgfmq}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig2_allSYNC_3PG_2x2}
\caption{All-machine response to the bus-14 reference fault: RMS voltage and current, active power, and reactive power at the 230~kV generator buses, on the transmission side of the step-up transformers. The generator-bus active powers oscillate at the 0.85--1.0~Hz inter-machine mode with decay rates of 0.06--0.10~s$^{-1}$. The legend lists the monitored buses; bus~14 is isolated by clearing.}
\label{fig:allsync}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{sweep_0_5_10_GFM_vrms}
\caption{Bus-voltage responses with zero, five, and ten GFM units under the reference fault. The GFM transient-current setting is 2.0 per unit; configurations are identified by replacement count.}
\label{fig:sweep}
\end{figure}
...
Figure~\ref{fig:ilimarms} compares the bus-14 reference fault on the all-GFM fleet at $I_{\max F}=2.0$, 1.5, and 1.2 per unit with all other settings unchanged. The fault is electrically remote from the units: at 2.0 per unit no unit reaches its limit, with peaks of 1.53, 1.44, and 1.38 per unit at buses~32, 30, and~31 and the other seven below 0.92 per unit. At 1.5 per unit only the bus-32 unit touches the limit, for 1.4~ms. At 1.2 per unit the three nearest units exceed the limit in the internal current-magnitude signal for 7--13~ms, less than one cycle, with peaks of 1.44, 1.38, and 1.31 per unit. In all three cases every unit rides through: post-fault active power returns to within 0.5\% of its pre-fault value and the frequency settles at 60.01~Hz.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{narr_ilim_arms_bus14}
\caption{Bus-14 reference fault on the all-GFM fleet at three transient current limits, 10~s records with identical dispatch, fault, and clearing. Left: peak of each unit's internal current magnitude (the inverter's own measurement at its terminals, 10~kV side of the step-up transformer) on its own base, with the three limit lines. Right: the same signal for the bus-32 unit through the fault.}
\label{fig:ilimarms}
\end{figure}

The control limiter is not the only protection a unit carries; a real inverter also has a hardware overcurrent trip that blocks the switches when the instantaneous current exceeds a device threshold. It was represented as follows: a three-pole breaker was placed in the 10~kV branch between each selected unit and its step-up transformer, controlled by logic on the unit's own instantaneous phase currents; when any phase exceeds a threshold, expressed as a multiple $k$ of the rated peak current $\sqrt{2}\,S_{\mathrm{rated}}/(\sqrt{3}\cdot 10~\mathrm{kV})$, continuously for longer than a dwell time, the breaker opens and latches. Everything else in the model, including the control limiter, is unchanged, and each trip run is compared with the corresponding record without the trip. Adding this trip to the three limited units changes the outcome: with $k=1.5$ or $1.2$ and a 1~ms dwell, all three open 3 to 4~ms after fault inception, because the phase current carries the fault-on direct-current offset and reaches 1.8 to 1.9 per unit while the controller's own current magnitude reads 1.3 to 1.4 per unit. The fleet loses 1.5~GW, 24\% of its output, during the fault. The remaining seven units do not return the network to its pre-fault state (Figure~\ref{fig:trip}): bus~16 settles at 0.70 per unit and the fleet output at 4.5~GW against 6.25~GW, a depressed-voltage operating point sustained by the voltage dependence of the loads (experiment \texttt{fault\_trip\_gfm}). At the bus-10 fault the same trip with $k=2.0$ and a 1~ms dwell removes only the bus-32 unit, which clearing isolates in any case, so the other nine units respond exactly as without it; with a 5~ms dwell it does not operate, since a single half-cycle excursion above 2.0 per unit lasts at most 3.6~ms. Coordination between the control limiter, the hardware trip threshold, and its dwell time therefore decides ride-through at this fault, not the limiter setting alone.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{narr_trip_bus14}
\caption{Bus-14 reference fault on the all-GFM fleet at $I_{\max F}=1.2$ per unit without a hardware trip and with a trip on the instantaneous phase current at units 30, 31, and~32 (1.5 and 1.2 per unit of rated peak current, 1~ms dwell). (a)~Bus-16 RMS voltage in per unit of 230~kV. (b)~Sum of the ten units' active power on the 230~kV side of their step-up transformers. (c)~Unit-32 phase currents in its 10~kV branch, in per unit of rated peak current, over the first milliseconds of the fault in the 1.5 per-unit record, with the two thresholds and the instant the breaker opens.}
\label{fig:trip}
\end{figure}

Figure~\ref{fig:peaki} compares the device-base peak currents, including a machine peak of 3.72 per unit at the corresponding bus. GFM current is shaped by the modeled current limit, whereas machine fault current depends strongly on electromagnetic reactances and the network. Figure~\ref{fig:exec} shows the bus-39 terminal quantities, where the GFM case recovers more quickly and the machine current is substantially larger. Different ratings, dispatch, and measurement locations remain relevant even when both traces are expressed per unit. In the GFM runs, the active and reactive power traces immediately after clearing contain a 2--4~Hz settling component and a 60~Hz component below 10\% of the deviation, the latter from the power calculation during the transient; the machine traces oscillate at the 0.85--1.0~Hz inter-machine mode.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{narr_peakI_pair}
\caption{Peak currents for the bus-10 disturbance, normalized on individual device bases. For the GFM units the quantity is the inverter's own current magnitude at its terminals, on the 10~kV side of the step-up transformer; for the machines it is the RMS current on the 230~kV side of the step-up transformer. The GFM transient-current setting is 1.5 per unit.}
\label{fig:peaki}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{exec_gfm_vs_sync_bus39_pu}
\caption{Bus-39 responses in the bus-10 study, measured on the 230~kV side of the unit's step-up transformer: all-machine (blue) and all-GFM (red), each on its own device base. The GFM case shows smaller fault current and faster settling; the two cases have different pre-fault power injections.}
\label{fig:exec}
\end{figure}

\subsection{All-Synchronous Machine Fault Ride Through}
\label{sec:syncbaseline}
The all-machine system rides through the same reference fault as the all-GFM system. Its generator-bus active powers oscillate at the 0.85--1.0~Hz inter-machine mode with decay rates of 0.06--0.10~s$^{-1}$ (damping ratio 1--2\%), so the oscillation persists for tens of seconds; the units nearest the fault, buses~31 and~32, also carry a faster, more strongly damped local component. The stronger damping associated with the selected GFM controls helps interpret the contrast, as developed in Section~\ref{sec:dynamics}. Network conditions, voltage controls, and limiting also affect the complete transient response.

\subsection{Grid-Following Inverter Fault Ride Through}
\label{sec:gfl}
We also evaluated a PNNL-based GFL implementation in the testbed. The surrounding grid recovers within approximately one second, but the local voltage remains depressed over the plotted interval in Figures~\ref{fig:gflpocket} and~\ref{fig:gfldeadlock}. The filtered voltage remains below the dip threshold and the active and reactive current commands fall to zero. These signals point to a control-state response rather than demonstrating semiconductor breakdown.
...
\caption{Voltage-dip and current-command signals in the original GFL implementation. Filtered voltage remains below the dip threshold, followed by zero active and reactive current commands.}
...
\caption{Unbalanced faults at bus~14, all-machine and all-GFM fleets, 10~s records with the reference timing and clearing. Left: bus-16 RMS voltage through the fault (SLG solid, LLG dashed). Right: peak RMS current of each unit on its own base, measured on the 230~kV side of its step-up transformer.}
\label{fig:unbal}
\end{figure}
...
\label{sec:smmodel}
The classical model relates electrical power imbalance to rotor acceleration \cite{sauer1998,kundur1994}. Let $\omega_0=2\pi f_{\mathrm{n}}$ denote nominal electrical angular frequency in rad/s, where $f_{\mathrm{n}}$ is nominal frequency in Hz. Define the normalized frequency deviation as $\nu=(\omega_{\mathrm{e}}-\omega_0)/\omega_0$ and electrical phase displacement $\delta$ relative to the synchronous reference by $\dot\delta=\omega_0\nu$. With powers normalized on the machine apparent-power base,
\begin{equation}
...
\begin{figure}[tbp]\centering
\input{figures/tikz_sm_basic_model.tex}
\caption{Classical synchronous-machine model: transient internal voltage $E^\prime\angle\delta$ behind reactance $X^\prime_{\mathrm{d}}$, connected to the grid at $V\angle\delta_{\mathrm{V}}$. The internal magnitude is set by the field circuit. The electrical phase displacement is tied to rotor motion and satisfies $\dot\delta=\omega_0\nu$.}
\label{fig:smcircuit}
\end{figure}

\subsection{GFM Inverter Dynamic Model}
\label{sec:gfmmodel}
The reduced GFM model is a controllable voltage source behind coupling reactance $X_{\mathrm{L}}$ \cite{du2023a1}. Its topology resembles the classical machine circuit, but the controller determines its internal voltage magnitude and phase. Figure~\ref{fig:gfmcircuit} shows the interface and Figure~\ref{fig:gfmequiv} shows the corresponding Norton equivalent used in a network solution. The equivalent circuit does not change the resource's GFM control classification.

The controlled source has magnitude $E$ and phase $\delta_{\mathrm{E}}$, while the terminal grid voltage has magnitude $V$ and phase $\delta_{\mathrm{V}}$. In the machine representation, the field circuit sets the internal voltage magnitude and rotor motion determines its phase. In the GFM representation, the power and voltage controls perform the analogous functions. The controller acts on filtered power measurements rather than directly applying its droop gain to instantaneous unfiltered power.

In the analyzed runs the REGFM\_A1 controls are the $P$--$f$ droop of Equation~\eqref{eq:drooplaw}, whose frequency is integrated to the internal angle $\delta_{\mathrm{E}}$; the $Q$--$V$ droop with the voltage regulator ($k_{\mathrm{pv}}=0$, $k_{\mathrm{iv}}=5.86$~pu/s, $E_{\max}=1.15$, VFlag~$=1$, regulating the point-of-interconnection voltage); the reactive limits $Q_{\max}=-Q_{\min}=0.44$; the overload controller ($k_{\mathrm{ppmax}}=0.01$, $k_{\mathrm{ipmax}}=0.1$~pu/s), which acts only while filtered power exceeds $P_{\max}$ ($P_{\max}=0.9$, $P_{\min}=0$ in the single-device and inertia-test cases; $P_{\max}=2.0$, $P_{\min}=-0.9$ in the 39-bus cases, where it never engages); and the transient current limit $I_{\max F}$. The plant-controller references $P_{\mathrm{ref}}$, $Q_{\mathrm{ref}}$, and $V_{\mathrm{ref}}$ are held at their initialized values. The model exposes no inertia constant (Figure~\ref{fig:pscada1params}); its equivalent inertia is the filtered-droop quantity $T_{\mathrm{Pf}}/2m_{\mathrm{p}}$ of Equation~\eqref{eq:heqdeq} only. No secondary control, that is, no slower loop that returns frequency and voltage to their reference values after the droop response (plant-level integral control or automatic generation control), acts in any run \cite{du2023a1}; the post-fault operating points are therefore the droop equilibria, and the effect of such a loop on stability is not assessed here. Figures~\ref{fig:controlblocks} and~\ref{fig:gfmblocks} show the machine governor path and the inverter controls as configured.
...
\begin{equation}
\nu=m_{\mathrm{p}}(p^\star-\tilde p),\qquad
E=E_0+m_{\mathrm{q}}(q^\star-\tilde q),
\label{eq:drooplaw}
\end{equation}
where $p^\star,q^\star$ are power references and $\tilde p,\tilde q$ are filtered power measurements. The active-power filter satisfies
\begin{equation}
T_{\mathrm{Pf}}\dot{\tilde p}=p-\tilde p,
\label{eq:pfilter}
\end{equation}
where $p$ is the unfiltered normalized active power. The controller-generated phase displacement satisfies $\dot\delta_{\mathrm{E}}=\omega_0\nu$ relative to the same synchronous reference. Table~\ref{tab:droopvars} defines the droop variables and their units.

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_basic_model.tex}
\caption{Reduced GFM inverter model, after Figure~1 of the REGFM\_A1 specification \cite{du2023a1}. A controllable voltage source $E\angle\delta_{\mathrm{E}}$ behind coupling reactance $X_{\mathrm{L}}$ delivers active and reactive power to the grid. Under unsaturated filtered droop, $\dot\delta_{\mathrm{E}}=\omega_0\nu$.}
\label{fig:gfmcircuit}
\end{figure}

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_equiv_circuits.tex}
\caption{Equivalent normalized circuits, after external Figure~2 of \cite{du2023a1}: internal voltage source behind reactance and its Norton representation. Here $I_{\mathrm{N}}\angle\varphi_{\mathrm{N}}$ is the equivalent injected current and $Y_{\mathrm{L}}=1/(jX_{\mathrm{L}})$ the parallel admittance; $\varphi$ denotes current phase. Solver representation does not determine GFM or GFL classification.}
\label{fig:gfmequiv}
\end{figure}

\begin{table}[tbp]\centering
\caption{Variables in Equations~\eqref{eq:drooplaw} and~\eqref{eq:pfilter}.}
\label{tab:droopvars}
\small
...
$\nu$ & Normalized electrical frequency deviation & p.u.\\
$E_0$ & Nominal internal-voltage reference & p.u.\\
$p^\star,q^\star$ & Active/reactive power references & p.u.\\
$\tilde p,\tilde q$ & Low-pass-filtered power measurements & p.u.\\
$\dot{\tilde p}$ & Filtered active-power time derivative & p.u./s\\
$m_{\mathrm{p}},m_{\mathrm{q}}$ & Frequency/power and voltage/reactive-power droop gains & p.u./p.u.\\
$T_{\mathrm{Pf}}$ & Active-power measurement-filter time constant & s\\\bottomrule
\end{tabular}
\end{table}

\begin{figure}[tbp]\centering
\resizebox{\textwidth}{!}{\input{figures/tikz_control_blocks.tex}}
\caption{GGOV1 governor and turbine model of the machine, redrawn block for block from Figure~3-5 of IEEE PES-TR1 \cite{pestr1} with the Table~\ref{tab:dyr_gov_params} settings and the figure's signal names. The speed error $P_{\mathrm{ref}}-\omega-R\,\tilde P_{\mathrm{e}}$, with Rselect~$=1$ selecting the electrical power $P_{\mathrm{e}}$ filtered by $T_{\mathrm{pelec}}$, is clipped to [minerr, maxerr] $=\pm0.055$ per unit, so a speed error larger than 0.055 per unit (3.3~Hz) produces no further governor action, and then drives the proportional gain and the integral branch; the integrator input $(K_{\mathrm{igov}}/K_{\mathrm{pgov}})(\mathit{fsr}-s_2)$ equals $K_{\mathrm{igov}}$ times the error while the governor is selected and tracks \textit{fsr} otherwise, which is the PES-TR1 anti-windup implementation of the PI controller. The load limiter ($L_{\mathrm{dref}}$, $K_{\mathrm{pload}}$, $K_{\mathrm{iload}}$, exhaust-temperature path through $T_{\mathrm{sa}}$, $T_{\mathrm{sb}}$, $T_{\mathrm{fload}}$) and the acceleration limiter ($A_{\mathrm{set}}$, $K_{\mathrm{a}}$, $T_{\mathrm{a}}$) use the same tracking structure; the Low Value Select block compares the three fuel demands, governor (\textit{fsrn}), load limiter (\textit{fsrt}) and acceleration limiter (\textit{fsra}), and passes the smallest to the valve as \textit{fsr}, so whichever controller asks for the least fuel is in control; whose stroke is rate-limited ($R_{\mathrm{open}}$, $R_{\mathrm{close}}$) and position-limited ($v_{\min}$, $v_{\max}$); with Flag~$=1$ the fuel flow \textit{Wf} is the stroke times speed, and $P_{\mathrm{mech}}=K_{\mathrm{turb}}(\mathit{Wf}-W_{\mathrm{fnl}})$ through the $T_{\mathrm{c}}$, $T_{\mathrm{b}}$ lead-lag. Grey blocks and wires are present in GGOV1 but switched off by the settings and carry no signal in any run: $K_{\mathrm{imw}}=0$, $K_{\mathrm{dgov}}=0$, $T_{\mathrm{eng}}=0$, $D_{\mathrm{m}}=0$. The integral term is $K_{\mathrm{igov}}$.}
\label{fig:controlblocks}
\end{figure}

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_controls.tex}
\caption{REGFM\_A1 outer controls, redrawn from Figures~3(a), 3(b), and~4 of the model specification \cite{du2023a1}, with the inputs $P_{\mathrm{inv}}$, $Q_{\mathrm{inv}}$, and $V_{\mathrm{inv}}$ already passed through the $T_{\mathrm{Pf}}$, $T_{\mathrm{Qf}}$, and $T_{\mathrm{Vf}}$ measurement filters. (a)~$P$--$f$ droop: $\Delta\omega=\omega_0\,[m_{\mathrm{p}}(P_{\mathrm{ref}}-P_{\mathrm{inv}})+\Delta\omega_{\mathrm{OL}}]$ is integrated to $\delta_{\mathrm{droop}}$; the overload mitigation term $\Delta\omega_{\mathrm{OL}}$ is nonzero only above $P_{\max}$ or below $P_{\min}$ because its two branches are limited to non-positive and non-negative values. (b)~$Q$--$V$ droop: with VFlag~$=1$, as used, the switch selects the proportional-integral regulator ($k_{\mathrm{pv}}=0$, $k_{\mathrm{iv}}=5.86$~pu/s) that drives the point-of-interconnection voltage $V_{\mathrm{inv}}$ to $V_{\mathrm{cmd}}$, with the output limited to $[E_{\min},E_{\max}]=[0,1.15]$; the $Q_{\max}/Q_{\min}$ branches add to $V_{\mathrm{cmd}}$ only beyond $\pm0.44$ per unit. (c)~Fault current limiting: while $I<I_{\max F}$ the internal voltage is the droop output; once $I\ge I_{\max F}$ it is recomputed from the terminal voltage and the coupling reactance so that the current magnitude is held at $I_{\max F}$ with its phase $\varphi$ unchanged. Integral terms: $k_{\mathrm{iv}}$, $k_{\mathrm{ipmax}}$, $k_{\mathrm{iqmax}}$.}
\label{fig:gfmblocks}
\end{figure}

\subsection{Modeling GFM Equivalent Inertia and Equivalent Damping Dynamics}
\label{sec:swingred}
For a fixed active-power reference, differentiating the first droop law relates frequency acceleration to the filtered-power derivative. Substituting Equation~\eqref{eq:pfilter} eliminates that derivative. Rearranging the power terms gives
\begin{align*}
\dot\nu&=-m_{\mathrm{p}}\dot{\tilde p}
=-\frac{m_{\mathrm{p}}}{T_{\mathrm{Pf}}}(p-\tilde p)\\
&=-\frac{m_{\mathrm{p}}}{T_{\mathrm{Pf}}}(p-p^\star)-\frac{\nu}{T_{\mathrm{Pf}}}.
\end{align*}
Multiplication by $T_{\mathrm{Pf}}/m_{\mathrm{p}}$ then gives
\begin{equation}
\underbrace{\frac{T_{\mathrm{Pf}}}{m_{\mathrm{p}}}}_{2H_{\mathrm{eq}}}\dot\nu
=p^\star-p-\underbrace{\frac{1}{m_{\mathrm{p}}}}_{D_{\mathrm{eq}}}\nu,
\qquad
\boxed{H_{\mathrm{eq}}=\frac{T_{\mathrm{Pf}}}{2m_{\mathrm{p}}},\quad D_{\mathrm{eq}}=\frac{1}{m_{\mathrm{p}}}.}
\label{eq:heqdeq}
\end{equation}
In terms of the relative inverter phase, this becomes
\begin{equation}
\frac{2H_{\mathrm{eq}}}{\omega_0}\ddot\delta_{\mathrm{E}}+
\frac{D_{\mathrm{eq}}}{\omega_0}\dot\delta_{\mathrm{E}}=p^\star-p.
\label{eq:gfmswing}
\end{equation}
...
\caption{Single-device fault response: voltage and current at the device terminals for the synchronous machine and GFM inverter; neither single-device case has a step-up transformer. The GFM transient-current setting is 2.0 per unit; both devices are dispatched at 0.50 per unit.}
\label{fig:smibvi}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.85\textwidth]{narr_smib_sync_vs_gfm_Pf}
\caption{Single-device active-power and frequency responses at matched 0.50 per-unit dispatch. The selected GFM model settles faster than the machine case. The dense band on the machine trace for about 0.5~s after clearing is a 60~Hz ripple of the instantaneous power calculation, produced by the direct-current offset of the armature current after the bolted fault; it decays with that offset and is not an oscillation of the machine. The machine's 1.5~Hz oscillation that follows decays at 1.6~s$^{-1}$ (Section~\ref{sec:smib}).}
\label{fig:smibpf}
\end{figure}

Figure~\ref{fig:smibipiq} separates active and reactive current contributions. The selected GFM model provides a rapid reactive-current response, while the machine response includes field-circuit dynamics. This comparison connects voltage support to device structure and controls. It does not imply identical current capabilities or response bandwidths across inverter designs.

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{smib_IPIQ}
\caption{Active and reactive current components for the single-device fault comparison, $I_{\mathrm{P}}=P/V$ and $I_{\mathrm{Q}}=Q/V$ on each device base, positive for injection into the grid, blanked while $V<0.2$ per unit. Both devices are dispatched at 0.50 per unit and both active powers return to 0.50 per unit; the active-current levels differ after clearing because $I_{\mathrm{P}}=P/V$ and the machine's terminal voltage settles at 1.06 per unit under its exciter while the inverter's is 1.00 per unit. The 60~Hz ripple on the machine traces is the armature direct-current offset described for Figure~\ref{fig:smibpf}. The inverter's reactive current is 0.4--0.6 per unit through the first 50~ms after clearing. The machine's reactive current is negative for 0.16~s after clearing, reaching $-0.65$ per unit at 3.12~s while its terminal voltage recovers from 0.95 to 1.02 per unit; its exciter then raises reactive power to 0.59 per unit at 1.06 per-unit voltage by 5~s.}
\label{fig:smibipiq}
\end{figure}

\subsection{Equivalent Damping, Inertia, and Settling Time}
\label{sec:dampinertia}
The common swing form allows comparison of the selected coefficients. For $m_{\mathrm{p}}=0.01$, Equation~\eqref{eq:heqdeq} gives $D_{\mathrm{eq}}=100$. This is constant within the reduced filtered-droop model, rather than a claim of frequency-independent physical response at every frequency. The machine's governor contribution depends on its dynamics and the perturbation frequency.

For the machine, the corresponding quantity is the in-phase part of the governor-and-turbine response, the term $D+\operatorname{Re}\{G(j\Omega)\}/R_{\mathrm{SG}}$ of the synchronous-generator operator in \cite[Eq.~(50)]{chatterjee2026note}, where $G$ is the normalized response from speed deviation to mechanical power ($G(0)=1$), $R_{\mathrm{SG}}$ is the permanent droop, and $\Omega$ is the perturbation angular frequency. Rather than a generic lag, $G$ is evaluated from the GGOV1 blocks of Figure~\ref{fig:controlblocks} with the Table~\ref{tab:dyr_gov_params} settings:
\begin{equation}
\frac{G(s)}{R_{\mathrm{SG}}}=-\frac{\Delta P_{\mathrm{mech}}}{\Delta\omega}
...
\label{eq:govpath}
\end{equation}
where $C(s)=K_{\mathrm{pgov}}+K_{\mathrm{igov}}/s$ is the governor controller ($K_{\mathrm{dgov}}=0$), $A(s)=1/(1+sT_{\mathrm{act}})$ the actuator, $T(s)=(1+sT_{\mathrm{c}})/(1+sT_{\mathrm{b}})$ the turbine lead-lag, $F(s)=1/(1+sT_{\mathrm{pelec}})$ the electrical-power feedback filter selected by Rselect~$=1$, and $W_{\mathrm{f0}}=W_{\mathrm{fnl}}+P_{\mathrm{m0}}/K_{\mathrm{turb}}$ the pre-disturbance fuel flow at dispatch $P_{\mathrm{m0}}$, which enters because Flag~$=1$ makes the fuel flow the product of valve stroke and speed. The electrical power fed back through $F(s)$ is that of a machine swinging against a stiff network, $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}-2Hs\,\Delta\omega$ from the swing equation, which gives the factor $(1-2HR_{\mathrm{SG}}sF)$ with $H=5.46$~s. That factor is 1 at zero frequency, so the steady-state value is $1/R_{\mathrm{SG}}=20$ whether or not the rotor's acceleration is included, but at 1~Hz it is $0.47-0.09j$ and removes about half of the direct speed path. In the 39-bus fleet $\Delta P_{\mathrm{e}}$ also carries the synchronizing power exchanged with the other nine machines, so Equation~\eqref{eq:govpath} is the single-machine form of the coefficient. $D=0$ in the GENROU record. The five-to-one comparison is therefore $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ for these normalized coefficients in the zero-frequency limit. It is not a universal ratio of total system damping. The stabilizer contributes nothing to this steady-state comparison: PSS2B acts on its inputs through high-pass (``washout'') stages with time constants $T_{\mathrm{w1}}=T_{\mathrm{w2}}=T_{\mathrm{w3}}=2$~s in the Appendix~\ref{app:params} record, whose gain at zero frequency is zero \cite[Sec.~12.5, p.~770]{kundur1994}, so $1/R_{\mathrm{SG}}$ is the machine's net steady-state droop. At the 0.85--1.0~Hz mode the relevant machine quantities are instead the governor-path value of $-1.5$ to $-1.8$ and the net damping 1.8 in Table~\ref{tab:dh}.

Figure~\ref{fig:damping} plots Equation~\eqref{eq:govpath} against perturbation frequency for the fleet's dispatch range, $P_{\mathrm{m0}}=0.50$ to $1.00$ per unit of each machine's rating (bus~39 at 0.50, the other nine at 0.90--1.00). The coefficient starts at 20, falls as the actuator and turbine lags accumulate phase, crosses zero between 0.38 and 0.41~Hz, and reaches $-1.4$ to $-1.8$ at 1~Hz. The phase lag of the path has then exceeded $90^\circ$, so the mechanical-power response is in antiphase with the speed deviation, which is negative damping in the damping-torque sense of \cite[Sec.~2.1, Eq.~(2.2); Sec.~12.5]{kundur1994}: a torque component in phase with the speed deviation is damping, and a component in antiphase is negative damping. The sign change comes from the two lags in the path, $T_{\mathrm{act}}=0.5$~s and $T_{\mathrm{b}}=0.1$~s; a single first-order lag, the stand-in of \cite[Eq.~(51)]{chatterjee2026note}, never reaches $90^\circ$ and cannot change sign. Closing the feedback with $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}$ instead, or omitting the speed dependence of the fuel flow, moves the crossing to 0.6--0.7~Hz and the 1~Hz value to between $-1.1$ and $-2.0$; the sign does not depend on either choice. Damper windings, excitation, the stabilizer, and network interactions also affect the total damping of a mode, and none of them is in Equation~\eqref{eq:govpath}.

The coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and projecting the mechanical torque on the speed at $f_{\mathrm{m}}$ (experiment \texttt{smib\_gov\_damping\_freqmod}). With the governor deadband removed from a copy of the record, the measured in-phase coefficient follows Equation~\eqref{eq:govpath} within 0.01 at 0.50 per unit ($+2.29$, $-0.86$, $-1.43$, $-1.42$, $-1.10$, and $-0.82$ at 0.3, 0.5, 0.7, 1.0, 1.5, and 2.0~Hz) and within 0.1 at 0.95 per unit, with the zero crossing between 0.3 and 0.5~Hz; these points are plotted in Figure~\ref{fig:damping}. With the supplied deadband of 0.0002 per unit the measured magnitude is 0.2 to 0.3 smaller at the test's 0.0015 per-unit speed swing, because the governor ignores the part of the speed error inside the deadband; at disturbance amplitudes the deadband is not a factor. The same test on the single-inverter case of Section~\ref{sec:smib} (dispatch 0.50 and 0.60 per unit, terminal power projected on the internal droop frequency) gives an in-phase coefficient of 99.9 at 0.3~Hz falling only to 97.0 at 2~Hz; the decrease is the 0.02~s smoothing of the power meter against the 0.01~s power-filter lead, $\mathrm{Re}[(1+j\omega T_{\mathrm{Pf}})/(1+0.02j\omega)]$, which predicts 99.2 at 1~Hz and 97.0 at 2~Hz, while the filtered droop power reproduces $1/m_{\mathrm{p}}=100$ exactly at every frequency; the inverter's coefficient is flat where the machine's changes sign.

The net machine damping in Table~\ref{tab:dh} is measured from the simulated response of the whole machine in the all-machine bus-14 case (experiment \texttt{fault\_3PG\_bus14\_0GFM}, 30~s record, Figure~\ref{fig:allsync}). After clearing, the ten generator-bus active powers oscillate together at the 0.85--1.0~Hz inter-machine mode and the oscillation decays; a Fourier transform of the 4--30~s record gives the frequency, and the logarithm of the ratio of successive peaks gives the decay rate $\sigma=0.06$--$0.10$~s$^{-1}$ for the eight machines away from the fault (buses 31 and 32 carry a faster local component and are excluded). For a mode of the swing equation the decay rate is $\sigma=D_{\mathrm{net}}/4H$, so $D_{\mathrm{net}}=4H\sigma\approx1.8$ (1.4--2.2) on the machine base with $H=5.46$~s. This net value includes everything that damps the mode; its excess over the governor path is not attributed to individual components. PSS2B and ESST4B are active in all machine cases; no case without the stabilizer was simulated. The positive GFM coefficient provides an important interpretation of the less oscillatory response, without attributing every transient difference to that coefficient alone.

The inertia comparison runs in the opposite direction. The study values $T_{\mathrm{Pf}}=10$~ms and $m_{\mathrm{p}}=0.01$ give $H_{\mathrm{eq}}=0.5$~s, roughly one eleventh of the machine's 5.46~s. Table~\ref{tab:dh} summarizes the comparison, keeping the governor contribution separate from the net damping measured from the machine's post-fault oscillation. Strong equivalent damping together with low equivalent inertia is consistent with the observed faster GFM settling.

\begin{table}[tbp]\centering
\caption{Selected inertia and damping quantities. Damping coefficients use normalized power per normalized frequency; the net machine value, measured from the decay of the post-fault oscillation in the all-machine bus-14 case, represents the complete response rather than the governor alone.}
\label{tab:dh}
\small
...
In-phase coefficient, 1~Hz, measured in PSCAD & 99.2 (0.50 and 0.60 pu) & $-1.42$ (0.50 pu), $-1.68$ (0.95 pu)\\
Governor zero crossing & Not applicable & 0.38--0.41~Hz\\
Net damping, post-fault oscillation & --- & 1.8 (1.4--2.2)\\
Post-fault response & Less oscillatory & Oscillatory, 0.85--1.0~Hz\\\bottomrule
\end{tabular}
\end{table}

\begin{figure}[tbp]\centering
\includegraphics[width=0.85\textwidth]{narr_damping_vs_freq}
\caption{Equivalent damping versus perturbation frequency, normalized power per normalized frequency. Analytic curves: the GFM filtered-droop coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}=100$ and the machine governor-and-turbine path of Equation~\eqref{eq:govpath}, which starts at $1/R_{\mathrm{SG}}=20$ and changes sign at 0.38--0.41~Hz (band: fleet dispatch 0.50 to 1.00 per unit; line: median dispatch; shading: where the path is negative, inside the 0.1--3~Hz swing band). Measured points: PSCAD frequency-modulation test on the single-machine case at 0.50 (open circles) and 0.95 per unit (filled squares) with the governor deadband removed, and on the single-inverter case at 0.50 and 0.60 per unit (red open circles, terminal power on the internal droop frequency); the diamond is the net damping of the whole machine from the post-fault oscillation of the all-machine bus-14 case, 1.8 (1.4--2.2) at 0.85--1.0~Hz.}
\label{fig:damping}
\end{figure}

\subsection{Measuring GFM Equivalent Inertia}
\label{sec:inertiaest}
The ERCOT PMView inertia estimation test applies a frequency ramp and calculates an estimated equivalent inertia $H_{\mathrm{est}}$ from the incremental power response. We used PMView~3.5 profile~17, associated with the Dynamics Working Group Procedure Manual Revision~24, Section~3.1.5.12 \cite{pmview,dwg24}. The test uses short-circuit ratio (SCR)~3, reactance-to-resistance ratio $X/R=6$, and a $-1$~Hz/s source-frequency ramp at nominal frequency 60~Hz. Over the 0.5~s integration window,
\begin{equation}
...
\caption{Contributions to the integrated power response during a constant rate-of-change-of-frequency ramp, from Equations~\eqref{eq:dpsplit} and~\eqref{eq:esplit}. The inertial contribution is constant and forms a rectangle; the damping contribution accumulates and forms a triangle. The curves illustrate the analytical decomposition.}
\label{fig:esplit}
\end{figure}
...
\caption{Illustrative time-scale separation between the inertial response and the droop response. The curves isolate the effect of droop-path lag: a faster path contributes within the measurement window, while a slower path contributes less over that interval.}
\label{fig:tscale}
\end{figure}
...
\caption{Inertia estimate from Equation~\eqref{eq:hest} compared with the PSCAD integration-window sweep at $m_{\mathrm{p}}=0.1$. The intercept is the equivalent inertia $T_{\mathrm{Pf}}/(2m_{\mathrm{p}})=0.05$~s and the slope is $1/(4m_{\mathrm{p}})=2.5$. The line predicts the inertia estimator output including damping, rather than an inertia estimate with damping removed. The dotted line is $H_{\mathrm{eq}}=0.05$~s.}
\label{fig:hestfit}
\end{figure}
...
\label{eq:subtract}
\end{equation}
Applying the same test normalization to this corrected energy yields $H$ under the stated assumptions. The subtraction must use consistent power bases and the frequency associated with the reduced model. Saturation, finite bandwidth, and network dynamics can prevent that separation.
...
\caption{PMView inertia estimate of the droop-controlled inverter versus short-circuit ratio, 0.5~s window, dispatch $P=0$. (a)~The estimate referred to the imposed $-1$~Hz/s ramp, as in Equation~\eqref{eq:agsesr}, and referred to the inverter's internal droop frequency over the same window; the dotted line is the analytic $H_{\mathrm{eq}}+T/(4m_{\mathrm{p}})$. (b)~Least-squares slope of the internal droop frequency over the window. Short-circuit ratios 3, 5, 10, and 20 are the records behind the earlier 7.44 to 10.06~s range; 2, 7, and 40 were added for this figure.}
\label{fig:hestscr}
\end{figure}
...
\caption{PMView inertia estimation test for the droop-controlled GFM inverter: $H_{\mathrm{est}}=7.44$~s. This estimate includes the controller response and is distinct from the filtered-droop equivalent inertia.}
\label{fig:hgfm}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.8\textwidth]{fig11_inertia_mechanism_pair}
\caption{Estimated equivalent inertia versus integration-window length for the tested control configurations. The window dependence illustrates the contribution of damping-related power response. Configured virtual inertia and filtered-droop equivalent inertia are distinct quantities.}
\label{fig:hmech}
\end{figure}
...
Figure~\ref{fig:efcct} summarizes the preliminary comparisons. For the single-machine case, the 0.474~s estimate is below the demonstrated stable duration of 0.622~s. For the 39-bus machine case, the 0.248~s estimate lies inside the $(0.237,0.250]$~s simulation bracket. An estimate inside a bracket is consistent with its resolution but is not, by itself, demonstrably conservative.

\begin{figure}[tbp]\centering
\includegraphics[width=0.8\textwidth]{ef_cct_machines_vs_gfm}
\caption{Energy-method critical clearing times (navy) compared with PSCAD EMT brackets (orange); each bracket gives a successful lower endpoint and an unsuccessful upper endpoint of the clearing duration, and the estimates are single values from the reduced models. Machine estimates use the classical model: constant transient voltage behind $x'_{\mathrm{d}}$, constant mechanical power, and no damping ($D=0$, no exciter, governor, or stabilizer); the single-machine case is lossless with zero electrical power during the bolted fault, and the 39-bus case keeps the network with constant-power loads. GFM estimates use the swing-form droop model of Equation~\eqref{eq:heqdeq}: constant internal voltage behind the filter and transformer reactance, equivalent inertia $H_{\mathrm{eq}}=T_{\mathrm{Pf}}/(2m_{\mathrm{p}})=0.5$~s, and the droop damping $D=1/m_{\mathrm{p}}=100$ included, but no current or voltage limit; the 39-bus case again has constant-power loads. Machine estimates are 0.474~s versus $(0.622,0.625]$~s for the single-machine case and 0.248~s versus $(0.237,0.250]$~s for the 39-bus case. GFM estimates are 1.296~s versus $(0.925,0.931]$~s and 2.338~s versus $(1.600,1.700]$~s, respectively; they exceed their brackets because the current limiter, which bounds the power a unit delivers during and after the fault, is absent from the reduced model.}
\label{fig:efcct}
\end{figure}
...
\begin{figure}[tbp]\centering
\input{figures/energy_surface_below_cct_coi.tex}
\caption{Potential-energy contours in equilibrium-shifted COI coordinates for the 39-bus system: (a)~ten synchronous machines and (b)~ten GFM inverters. Clearing durations are the last stable cases, 0.237~s and 1.600~s, respectively. White dotted and thin solid curves show the model's fault-on and post-clearing paths; cyan dashed curves show PSCAD EMT trajectories. The thick white curve is the model boundary estimate. Circles mark clearing, squares mark model endpoints, and cyan crosses mark EMT endpoints. The colour scale is shared by both panels.}
\label{fig:esbelow}
\end{figure}
...
\input{figures/energy_surface_at_cct_coi.tex}
\par\vspace{4pt}
\input{figures/energy_surface_at_cct_3d.tex}
\caption{Complementary views near the analytical critical clearing times: (a,b)~2D potential-energy contours and (c,d)~3D surfaces, with synchronous machines on the left and GFM inverters on the right. Clearing durations are 0.2455~s and 2.315~s; model estimates are 0.248~s and 2.338~s. All panels use the equilibrium-shifted COI angles in Equation~\eqref{eq:energycoi} and the shared legend above the 3D views. The GFM EMT endpoint lies in a neighboring well, unlike the model endpoint near the origin. EMT clearing-time brackets remain $(0.237,0.250]$~s and $(1.600,1.700]$~s.}
\label{fig:esat}
\end{figure}
...
The other quantity the inverter estimate depends on is the droop damping itself. Figure~\ref{fig:dampsweep} repeats the two inverter estimates with the damping $D=1/m_{\mathrm{p}}$ scaled from zero to its full value, everything else unchanged. With the damping removed the reduced inverter is a 0.5~s inertia and clears in 0.140~s (single inverter; the undamped equal-area value) and 0.161~s (ten inverters); with the full droop damping the same models give 1.297~s and 2.338~s. The published inverter estimates are therefore damping-set drift times, in which the angle reaches the unstable equilibrium slowly and with almost no kinetic energy, not inertial first swings. The EMT brackets lie between the undamped and the damped values because neither reduced model carries the current limiter that sets the measured clearing time. The damped values remain the ones compared in Figure~\ref{fig:efcct}: setting $D=0$ is the classical-machine convention, whose model has no damping to keep, and not a description of a droop inverter, whose damping is its primary control term. The undamped ten-inverter value is an order of magnitude only, since the constant-power network solution fails repeatedly during the fast undamped swings.

\begin{figure}[tbp]\centering
\includegraphics[width=0.85\textwidth]{narr_cct_damping_sweep}
\caption{Energy-method critical clearing time of the reduced droop-inverter models as the droop damping is scaled from zero ($\alpha=0$, drawn at the left edge) to its full value ($\alpha=1$, the device of Figure~\ref{fig:efcct}), with the equivalent inertia $T_{\mathrm{Pf}}/(2m_{\mathrm{p}})=0.5$~s, the constant internal voltage, the constant-power loads, and the absence of a current limiter unchanged. Circles: single inverter on the infinite bus. Squares: ten inverters in the 39-bus network with the bus-14 fault. Shaded bands: the PSCAD EMT brackets of the same two cases.}
\label{fig:dampsweep}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.9\textwidth]{ilimit_sweep_emt}
\caption{Measured critical clearing time of the single inverter on the infinite bus as a function of the transient current limit $I_{\max F}$, all other settings fixed. Each bar is the bracket between the longest stable and the shortest unstable fault duration.}
\label{fig:ilimitsweep}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=0.9\textwidth]{saturation_block}
\caption{Conceptual effect of current saturation on the power-angle characteristic. The limited characteristic departs from the sinusoid, changing the location of the unstable equilibrium and the region of attraction represented by the classical construction.}
\label{fig:satblock}
\end{figure}
...
\begin{itemize}
\item \textbf{Limiter-aware stability estimates.} Extend critical-clearing-time analysis to include saturation and determine conditions under which the estimate is conservative.
...
\item \textbf{Mixed-generation systems.} Extend the analysis to combinations of synchronous machines, GFM inverters, and GFL inverters.
...
\item \textbf{Protection coordination and damping decomposition.} The hardware overcurrent trip test of Section~\ref{sec:ilim} showed that the limiter setting alone does not decide ride-through: at the bus-10 fault the trip removes only the unit that clearing isolates, but at the bus-14 fault with the 1.2 per-unit limiter it removes three units 3 to 4~ms into the fault and leaves the network at a depressed voltage. The remaining work is to coordinate the limiter setting, the trip threshold, and the dwell time with device data, and to examine the load model's behavior at the resulting voltages. Separately, separate the machine's net damping into governor, stabilizer, and field contributions through post-fault oscillation records with the governor frozen and with the stabilizer disabled.
...
\section{Alternative GFM Model Diagnostics}
\label{app:altmodels}
This appendix records the model-quality-test diagnostics of the alternative GFM implementations evaluated in Section~\ref{sec:mqt}. In selected stiff-grid tests of the NLR GFM model, both high-voltage ride-through and both phase-angle-jump tests exhibited an approximately 5~Hz oscillation; repeated limiter transitions suggest an interaction with the voltage regulator in this implementation and configuration. Figure~\ref{fig:pscadkenyon} shows its control parameters.

Figure~\ref{fig:mqtpair} compares REGFM\_A1 and the NLR GFM model under the same 1.02 per-unit source-voltage step. Figure~\ref{fig:mqtvstepdown} shows the downward step, for which REGFM\_A1 settles more quickly while the NLR GFM model develops an oscillatory response. Figure~\ref{fig:mqtlimiter} shows the associated limiting behavior in a high-voltage test. These results characterize the selected implementations and settings rather than all GFM designs.

The downward-step test changes the source voltage from 1.00 to 0.97 per unit at 5~s. REGFM\_A1 settles within approximately a quarter second, while the NLR GFM model develops a growing oscillation. In the high-voltage test, the plotted current components repeatedly encounter their limiting ceiling as the voltage-loop error changes sign. The limiter-status trace records 203 transitions before the response diverges near 23~s, suggesting an interaction between the integral voltage regulator and the limiting action.

\begin{figure}[tbp]\centering
\includegraphics[width=0.6\textwidth]{pscad_nlr_params}
\caption{NLR GFM model control parameters, including the voltage-regulator gains.}
\label{fig:pscadkenyon}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig3_mqt_pnnl_vs_nlr}
\caption{REGFM\_A1 and NLR GFM model responses to a 1.02 per-unit source-voltage step in PMView.}
\label{fig:mqtpair}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig3_mqt_vstep_down}
\caption{Point-of-interconnection voltage after an identical 1.00 to
0.97~p.u. step at $t=5$~s in the PMView harness: PNNL REGFM\_A1 (left)
settles within a quarter second, while the NLR GFM model (right)
enters a growing voltage oscillation.}
\label{fig:mqtvstepdown}
\end{figure}

\begin{figure}[tbp]\centering
\includegraphics[width=\textwidth]{fig3_mqt_nlr_limiter_chatter}
\caption{NLR GFM model high-voltage test: voltage error (top), current magnitude and limit (middle), and limiter state (bottom). Repeated limiter transitions accompany the oscillatory response before its change near 23~s.}
\label{fig:mqtlimiter}
\end{figure}


\section{Constants of the Energy-Method Models}
\label{app:constants}
This appendix lists the quantities that enter the accelerating power of Equation~\eqref{eq:fi} in the four cases of Figure~\ref{fig:efcct}; the inertia and damping coefficients are in Table~\ref{tab:dh} and Section~\ref{sec:energymethod}.

\paragraph{Single machine on the infinite bus.} Lossless, so $G_{12}=0$. The transient voltage $E'=1.111$ per unit behind $X'_{\mathrm{d}}=0.26$ per unit, the source $V=1.001$ per unit behind the identified external reactance $X_{\mathrm{ext}}=0.108$ per unit, all on the 2000~MVA machine base, give $B_{12}=1/(X'_{\mathrm{d}}+X_{\mathrm{ext}})=2.72$ per unit, $p_{\max}=E'V B_{12}=3.03$ per unit, and $p_{\mathrm{m}}=0.503$ per unit ($\delta_{\mathrm{s}}=9.6^\circ$). The reactance was identified from the measured pre-fault and post-fault terminal states of the PSCAD record.

\paragraph{Single inverter on the infinite bus.} Lossless, $G_{12}=0$; nameplate $E=V=1$ per unit, $X_{\mathrm{L}}=0.15$ per unit plus the grid reactance $X_{\mathrm{g}}=0.025$ per unit (short-circuit ratio 40) on the inverter base, so $B_{12}=1/0.175=5.71$ per unit, $p_{\max}=5.71$ per unit, and $p^\star=0.6$ per unit ($\delta_{\mathrm{s}}=6.0^\circ$).

\paragraph{39-bus fleets.} Table~\ref{tab:constants} gives, for each generator bus, the rating on which the device is represented, the coupling reactance on the 100~MVA system base ($X'_{\mathrm{d}}=0.26$ per unit on the machine rating; $X_{\mathrm{L}}=0.15$ per unit on the inverter base $S_{\mathrm{inv,base}}=P_{\mathrm{G}}/0.6$), the internal voltage from the pre-fault load flow, and two entries of the pre-fault network reduced to the ten internal nodes with the loads represented as constant impedances at their pre-fault voltages: the magnitude of the self-susceptance $B_{ii}$ and the largest coupling susceptance $|B_{ij}|$ to another internal node. The dispatch $P_{\mathrm{m},i}$ is the same in both fleets (2.50 to 10.00 per unit on the system base, Table~\ref{tab:cases}). The transfer conductances are small in both fleets, $0\le G_{ij}\le 1.05$ per unit off the diagonal, and are retained in the calculations. The critical-clearing-time and surface calculations of Section~\ref{sec:energymethod} keep the constant-power loads rather than this reduced matrix, so the table is the linearized picture of the coupling; the full matrices are provided with the figure data. The largest coupling susceptance of an inverter node is 1.6 to 2.3 times the machine value at nine of the ten buses and 1.4 times at bus 39, which is the origin of the larger curvature of the inverter surface.

\begin{table}[tbp]\centering\footnotesize
\caption{Reduced-network constants of the two 39-bus fleets, per unit on the 100~MVA system base except the ratings (MVA columns). $B_{ii}$ and $\max_j|B_{ij}|$ from the pre-fault network reduced to the internal nodes with constant-impedance loads.}
\label{tab:constants}
\begin{tabular}{r rrrrr rrrrr}\toprule
 & \multicolumn{5}{c}{Synchronous machine} & \multicolumn{5}{c}{Grid-forming inverter}\\
\cmidrule(lr){2-6}\cmidrule(lr){7-11}
Bus & MVA & $X'_{\mathrm{d,sys}}$ & $E'_i$ & $|B_{ii}|$ & $\max_j|B_{ij}|$ & MVA & $X_{\mathrm{L,sys}}$ & $E_i$ & $|B_{ii}|$ & $\max_j|B_{ij}|$\\\midrule
30 & 250 & 0.1040 & 1.182 & 7.2 & 1.7 & 417 & 0.0360 & 1.091 & 14.6 & 4.0\\
31 & 600 & 0.0433 & 1.094 & 11.7 & 2.8 & 997 & 0.0150 & 1.004 & 18.3 & 5.5\\
32 & 650 & 0.0400 & 1.092 & 12.5 & 2.6 & 1083 & 0.0139 & 1.005 & 19.5 & 5.5\\
33 & 650 & 0.0400 & 1.069 & 13.2 & 3.4 & 1053 & 0.0142 & 1.014 & 21.6 & 7.7\\
34 & 525 & 0.0495 & 1.114 & 10.2 & 3.4 & 847 & 0.0177 & 1.044 & 16.1 & 7.7\\
35 & 650 & 0.0400 & 1.152 & 12.8 & 3.4 & 1083 & 0.0139 & 1.078 & 20.7 & 7.4\\
36 & 560 & 0.0464 & 1.101 & 10.1 & 3.4 & 933 & 0.0161 & 1.080 & 15.8 & 7.4\\
37 & 550 & 0.0473 & 1.049 & 10.8 & 2.5 & 900 & 0.0167 & 1.029 & 17.0 & 4.0\\
38 & 850 & 0.0306 & 1.064 & 9.8 & 1.7 & 1383 & 0.0108 & 1.033 & 12.4 & 2.9\\
39 & 2000 & 0.0130 & 1.040 & 18.2 & 2.8 & 1667 & 0.0090 & 1.037 & 20.7 & 3.9\\
\bottomrule
\end{tabular}
\end{table}
...
GFM Inverter Models} (software release~V1, includes
REGFM\_A1, REGFM\_B1, REGFM\_C1), Pacific Northwest National
Laboratory, May 20, 2026.
\url{https://github.com/pnnl/PSCAD-and-PSSE-Version-of-WECC-GFM-Inverter-Models}
...
\bibitem{regfma1lib} Pacific Northwest National Laboratory, \emph{REGFM\_A1 PSCAD model release (REGFM\_A1.zip, containing PNNL\_REGFM\_A1\_gf46.lib)}, GitHub release V1 of \emph{PSCAD and PSS/E Version of WECC GFM Inverter Models}. \url{https://github.com/pnnl/PSCAD-and-PSSE-Version-of-WECC-GFM-Inverter-Models/releases/download/V1/REGFM_A1.zip}
