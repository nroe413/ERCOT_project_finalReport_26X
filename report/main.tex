...
We developed a reusable simulation testbed to study power systems with a high share of generation provided by grid-forming (GFM) inverters. The University of Texas at Austin collaboration with the Electric Reliability Council of Texas (ERCOT) began with an IEEE 39-bus PSS/E network and translated the 39-bus network to PSCAD through the E-TRAN software package. We equipped the IEEE-39-bus network with representative machine dynamics supplied by ERCOT and with inverter models developed by Pacific Northwest National Lab (PNNL) for machine replacement studies. The first-year work primarily concerns electromagnetic transient (EMT) simulations in PSCAD; the PSS/E starting case provides the network foundation for corresponding positive-sequence studies in the next phase.

Battery storage motivates this work. Batteries can absorb energy when supply is abundant and inject energy on short notice when needed. Variable renewable generation and rapidly changing loads increase the importance of batteries' flexibile generation profile. ERCOT's advanced grid-support requirements for new inverter-based storage resources also make GFM behavior increasingly relevant to system studies. Understanding how these controls behave in steady state, during faults, and in the post-fault period is therefore important.

The testbed combines configurable network disturbances, resource models, and scripts that process the simulation outputs. We developed eleven configurations by replacing zero through ten synchronous machines with the selected droop-controlled GFM model.  The testbed also supports later studies of other GFM controls, grid-following (GFL) resources, mixed generation, user-defined load profiles, and load disturbances. The results are specific to this model and the tested cases: one droop-controlled GFM implementation and a limited set of disturbances at two buses. Other GFM controls, other settings of this one, or other scenarios may behave differently.
...
The analytical comparison connects these observations to familiar machine dynamics. A synchronous machine has a physical rotor whose motion determines its electrical phase. A GFM controller generates an electrical phase without a rotating shaft. Under unsaturated filtered droop, the two models can be written in the same reduced swing-equation form. The GFM droop gain sets an equivalent damping coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}$, while the power-measurement filter and droop gain set equivalent inertia. For the selected parameters, the low-frequency damping coefficient is five times the machine governor droop coefficient on the corresponding normalized bases.

This equivalent damping provides an important interpretation of the strongly damped GFM responses, but it is not the only control property that affects fault recovery. Current and voltage limits change the power that an inverter can deliver during severe disturbances. The inertia estimation tests further show that damping-related power contributes to the estimated inertia over the test window. The preliminary transient stability energy-method investigation examines how grid-reduced dynamics compare with EMT fault-duration limits.

The next phase will align topology, operating points, ratings, bases, applicable controls, and disturbances in corresponding PSS/E and PSCAD studies. The objective is to understand differences between modeling domains. We will also extend the preliminary analytical work to investigate how current limits, voltage limits, and droop settings influence transient stability. Together, the testbed and analytical interpretation provide a foundation for guidance to manufacturers and power system engineers.
...
The control limiter is not the only protection a unit carries. Adding a hardware trip on the instantaneous phase current to the three limited units changes the outcome: with a threshold of either 1.5 or 1.2 per unit of rated peak current and a 1~ms dwell, all three open 3 to 4~ms after fault inception, because the phase current carries the fault-on direct-current offset and reaches 1.8 to 1.9 per unit while the controller's own current magnitude reads 1.3 to 1.4 per unit. The fleet loses 1.5~GW, 24\% of its output, during the fault. The remaining seven units do not return the network to its pre-fault state (Figure~\ref{fig:trip}): bus~16 settles at 0.70 per unit and the fleet output at 4.5~GW against 6.25~GW, a depressed-voltage operating point sustained by the voltage dependence of the loads (experiment \texttt{fault\_trip\_gfm}; the breaker sits in each unit's 10~kV branch, and the trip latches once any phase exceeds the threshold for longer than the dwell). At the bus-10 fault the same trip at 2.0 per unit of rated peak for 1~ms removes only the bus-32 unit, which clearing isolates in any case, so the other nine units respond exactly as without it; with a 5~ms dwell it does not operate, since a single half-cycle excursion above 2.0 per unit lasts at most 3.6~ms. Coordination between the control limiter, the hardware trip threshold, and its dwell time therefore decides ride-through at this fault, not the limiter setting alone.
