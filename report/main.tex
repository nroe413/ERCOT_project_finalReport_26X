...
\caption{All-machine response to the bus-14 reference fault: RMS voltage and current, active power, and reactive power at the 230~kV generator buses, on the transmission side of the step-up transformers. The generator-bus active powers oscillate in two inter-machine modes at 0.85 and 1.02~Hz with decay rates of 0.08--0.10~s$^{-1}$ (Figure~\ref{fig:ringdown}). The legend lists the monitored buses; bus~14 is isolated by clearing.}
\label{fig:allsync}
\end{figure}
...
The all-machine system rides through the same reference fault as the all-GFM system. Its generator-bus active powers oscillate in two inter-machine modes at 0.85 and 1.02~Hz with decay rates of 0.08--0.10~s$^{-1}$ (damping ratio about 1.5\%; Section~\ref{sec:dampinertia}, Figure~\ref{fig:ringdown}), so the oscillation persists for tens of seconds; the units nearest the fault, buses~31 and~32, are dominated by a faster local component that decays at about 0.35~s$^{-1}$. The stronger damping associated with the selected GFM controls helps interpret the contrast, as developed in Section~\ref{sec:dynamics}. Network conditions, voltage controls, and limiting also affect the complete transient response.
...
The net machine damping in Table~\ref{tab:dh} comes from the all-machine bus-14 case (30~s record, Figure~\ref{fig:allsync}). After clearing, the generator-bus active powers oscillate and decay (Figure~\ref{fig:ringdown}). Each record, band-passed to 0.5--2~Hz, was fitted over 6--30~s with a sum of damped sinusoids by the matrix-pencil method. Two inter-machine modes, at 0.85 and 1.02~Hz, dominate the six units away from the fault (buses 33--36, 38, and 39) and decay at $\sigma=0.075$--$0.104$~s$^{-1}$; the units at buses 31 and 32, next to the fault, and the small-amplitude units at buses 30 and 37 are dominated by faster local components (1.1--1.3~Hz, $\sigma\approx0.3$--$0.4$~s$^{-1}$), which are excluded. With the electrical power responding to the angle through the synchronizing coefficient, $\Delta P_{\mathrm{e}}=K_{\mathrm{S}}\Delta\delta$ and $\Delta\dot\delta=\omega_0\Delta\bar{\omega}$, the swing equation above becomes
\[
...
Net damping, post-fault oscillation & --- & 1.8 (1.6--2.3)\\
