...
\caption{Bus-14 reference fault on the all-GFM fleet at $I_{\max F}=1.2$ per unit without a hardware trip and with a trip on the instantaneous phase current at units 30, 31, and~32 (1.5 and 1.2 per unit of rated peak current, 1~ms dwell). (a)~Bus-16 RMS voltage in per unit of 230~kV. (b)~Sum of the ten units' active power on the 230~kV side of their step-up transformers. (c)~Unit-32 phase currents in its 10~kV branch, in per unit of rated peak current, over the first milliseconds of the fault in the 1.5 per-unit record, with the 1.5 per-unit threshold and the instant the breaker opens.}
\label{fig:trip}
\end{figure}
...
Governor zero crossing & Not applicable & 0.38--0.41~Hz\\\bottomrule
\end{tabular}
\end{table}
