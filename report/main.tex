...
\begin{table}[tbp]\centering\small
\caption{Current-limit cases on the all-GFM fleet (Figures~\ref{fig:ilimarms} and~\ref{fig:ilimarms13}): the units at buses~30, 31, and~32, the three nearest the fault. Peak is the maximum of the inverter's internal current magnitude. Time above the limit is the total within one second of fault inception. Back at the limit is the time from fault inception until the current is within 2\% of the limit and stays there for a full cycle. A dash means the limit was not reached; the 15 per-unit case has no practical limit. The other seven units stay below 0.92 per unit in every case.}
\label{tab:ilimarms}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{cc*{9}{>{\centering\arraybackslash}p{0.9cm}}c}\toprule
 & & \multicolumn{3}{c}{Peak (p.u.)} & \multicolumn{3}{c}{Above limit (cycles)} & \multicolumn{3}{c}{Back at limit (cycles)} & \\
