...
\item The \emph{equivalent inertia} of filtered droop arises from the lag in its power measurement and depends on the filter time constant and droop gain \cite{chatterjee2026note,chatterjee2025eac}. A longer filter time constant or a smaller droop gain raises this coefficient, and with it the inverter angle's resistance to acceleration, without any physical moment of inertia changing. The electrical energy still comes from the DC source and circuit storage.
...
\caption{Classical synchronous-machine model: transient internal voltage $E^\prime\angle\theta_{\mathrm{E}}$ behind reactance $X^\prime_{\mathrm{d}}$, connected to the grid at $V\angle\theta_{\mathrm{V}}$. The internal magnitude is set by the field circuit. The displacement $\delta=\theta_{\mathrm{E}}-\theta_{\mathrm{V}}$ is tied to rotor motion and satisfies $\dot\delta=\omega_0\Delta\bar{\omega}$.}
\label{fig:smcircuit}
\end{figure}
...
\label{eq:pfilter}
\end{equation}
where $p$ is the unfiltered normalized active power. The controller-generated phase satisfies $\dot\theta_{\mathrm{E}}=\omega_0\Delta\bar{\omega}$ relative to the same synchronous reference, so against the stiff grid $\dot\delta=\omega_0\Delta\bar{\omega}$ as for the machine. Table~\ref{tab:droopvars} defines the droop variables and their units.

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_basic_model.tex}
\caption{Reduced GFM inverter model, after Figure~1 of the REGFM\_A1 specification \cite{du2023a1}. A controllable voltage source $E\angle\theta_{\mathrm{E}}$ behind coupling reactance $X_{\mathrm{L}}$ delivers active and reactive power to the grid at $V\angle\theta_{\mathrm{V}}$. Under unsaturated filtered droop, $\dot\theta_{\mathrm{E}}=\omega_0\Delta\bar{\omega}$.}
...
\frac{D_{\mathrm{eq}}}{\omega_0}\dot\delta=p^\star-p.
\label{eq:gfmswing}
\end{equation}
...
\bibitem{ieee1057} IEEE Standards Association, \emph{IEEE Standard for Digitizing Waveform Recorders}, IEEE Std 1057-2017, January 2018, doi:10.1109/IEEESTD.2018.8291741.
