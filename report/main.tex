...
\item \textbf{Protection and damping decomposition.} The bus-10 study already shows the fleet riding through the loss of the unit nearest the fault, which is disconnected at clearing and carries no current afterward (Section~\ref{sec:ilim}); what remains untested is a hardware overcurrent trip that acts during the fault, within a cycle of the phase current exceeding a device threshold, at the units that reach their limits (bus~32 at 2.0 per unit of rated peak in the bus-10 case, buses 30--32 at the 1.2 per-unit setting). Add that trip to the inverter model with the threshold taken from device data, rerun the bus-10 and bus-14 faults, and separate the machine's net damping into governor, stabilizer, and field contributions through post-fault oscillation records with the governor frozen and with the stabilizer disabled.
