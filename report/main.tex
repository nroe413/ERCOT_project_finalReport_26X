...
The coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and evaluating Equation~\eqref{eq:dmeas} at $f_{\mathrm{m}}$. The complex amplitudes $\hat P_{\mathrm{mech}}$ and $\hat\omega$ are those of the two signals at $f_{\mathrm{m}}$ over the last 10~s of each record, trimmed to whole modulation cycles, on a shared time axis so that their phases can be subtracted. The machine's mechanical power follows from the recorded torque, $\hat P_{\mathrm{mech}}=\hat T_{\mathrm{mech}}+T_{\mathrm{m0}}\hat\omega$ in per unit with $T_{\mathrm{m0}}$ the pre-modulation torque; for the inverter the terminal power and the internal droop frequency take the places of $\hat P_{\mathrm{mech}}$ and $\hat\omega$.
