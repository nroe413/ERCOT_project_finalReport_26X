...
Four models were evaluated: REGFM\_A1, REGFM\_B1, the NLR GFM model, and the PSCAD library GFM model. REGFM\_A1 was selected for the system studies as the configuration usable as shipped; the other models were not tuned further.
...
The other quantity the inverter estimate depends on is the droop damping itself. Figure~\ref{fig:dampsweep} repeats the two inverter estimates with the damping $D=1/m_{\mathrm{p}}$ scaled from zero to its full value, everything else unchanged. With the damping removed the reduced single inverter is a 0.5~s inertia and clears in 0.140~s (the undamped equal-area value); with the full droop damping the same models give 1.297~s (single inverter) and 2.338~s (ten inverters), and the ten-inverter estimate falls to 0.279~s at one tenth of the droop damping. The published inverter estimates are therefore damping-set drift times, in which the angle reaches the unstable equilibrium slowly and with almost no kinetic energy, not inertial first swings. The EMT brackets lie between the undamped and the damped values because neither reduced model carries the current limiter that sets the measured clearing time. The damped values remain the ones compared in Figure~\ref{fig:efcct}: setting $D=0$ is the classical-machine convention, whose model has no damping to keep, and not a description of a droop inverter, whose damping is its primary control term. The ten-inverter sweep is shown from one tenth of the droop damping upward: at smaller scales the fleet swings fast and wide and the constant-power network solution failed in more than ten thousand time steps of each bisection, against a few hundred at the plotted scales, so those runs are not reported.

\begin{figure}[tbp]\centering
\includegraphics[width=0.85\textwidth]{narr_cct_damping_sweep}
\caption{Energy-method critical clearing time of the reduced droop-inverter models as the droop damping is scaled from zero ($\alpha=0$, drawn at the left edge) to its full value ($\alpha=1$, the device of Figure~\ref{fig:efcct}), with the equivalent inertia $T_{\mathrm{Pf}}/(2m_{\mathrm{p}})=0.5$~s, the constant internal voltage, the constant-power loads, and the absence of a current limiter unchanged. Circles: single inverter on the infinite bus. Squares: ten inverters in the 39-bus network with the bus-14 fault, at damping scales of 0.1 and above (see text). Shaded bands: the PSCAD EMT brackets of the same two cases.}
\label{fig:dampsweep}
\end{figure}
