...
\caption{Equivalent damping versus perturbation frequency. The filtered-droop coefficient is $D_{\mathrm{eq}}=100$; the machine governor-and-turbine path of Equation~\eqref{eq:govpath} starts at $1/R_{\mathrm{SG}}=20$ and its in-phase component changes sign at 0.38--0.41~Hz; the band spans the fleet dispatch of 0.50 to 1.00 per unit and the line is the median dispatch. Open circles and filled squares are the PSCAD frequency-modulation measurements on the single-machine case at 0.50 and 0.95 per unit with the governor deadband removed. Coefficients use normalized power per normalized frequency. The diamond is the net damping of the whole machine measured from the decay of the post-fault oscillation in the all-machine bus-14 case, 1.8 (1.4--2.2), at its 0.85--1.0~Hz mode.}
\label{fig:damping}
\end{figure}
...
\item \textbf{Protection and damping decomposition.} Add an instantaneous hardware overcurrent trip to the inverter model, with the threshold taken from device data, and separate the machine's net damping into governor, stabilizer, and field contributions through post-fault oscillation records with the governor frozen and with the stabilizer disabled.
