...
The coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and evaluating Equation~\eqref{eq:dmeas} at $f_{\mathrm{m}}$. The amplitudes come from a lock-in fit: each signal $x(t)$ is fitted by least squares \cite{ieee1057} over the last 10~s of the record, trimmed to whole modulation cycles, as
\[
x(t)\approx c_0+a_x\cos(2\pi f_{\mathrm{m}}t)+b_x\sin(2\pi f_{\mathrm{m}}t),\qquad \hat X=a_x-jb_x,
\]
which picks the component at the test frequency out of the ripple in the record, and the two signals share a time axis so that their phases can be subtracted. The machine's mechanical power follows from the recorded torque, $\hat P_{\mathrm{mech}}=\hat T_{\mathrm{mech}}+T_{\mathrm{m0}}\hat\omega$ in per unit with $T_{\mathrm{m0}}$ the pre-modulation torque; for the inverter the terminal power and the internal droop frequency take the places of $\hat P_{\mathrm{mech}}$ and $\hat\omega$.

The measured coefficient, with the governor as supplied, is $+0.50$, $-0.87$, $-1.09$, $-1.11$, $-0.97$, and $-0.56$ at 0.3, 0.5, 0.7, 1.0, 1.5, and 2.0~Hz at 0.50 per unit and $+0.02$, $-1.33$, $-1.39$, $-1.80$, $-1.31$, and $-0.89$ at 0.95 per unit; these are the measured points in Figure~\ref{fig:damping}. They follow the shape of Equation~\eqref{eq:govpath} and cross zero between 0.3 and 0.5~Hz, and they are mostly smaller in magnitude, by up to 0.5 between 0.5 and 2~Hz and by about 1.8 at 0.3~Hz, because the governor's speed deadband of 0.0002 per unit removes part of the test's 0.0015 per-unit speed swing; with the deadband set to zero in a copy of the model, the same test reproduces the equation within 0.01 at 0.50 per unit. The same test on the single-inverter case (dispatch 0.50 and 0.60 per unit) gives 99.9 at 0.3~Hz falling to 97.0 at 2~Hz; the decrease is the 0.02~s smoothing of the power meter against the 0.01~s power-filter lead, $\mathrm{Re}[(1+j\omega T_{\mathrm{Pf}})/(1+0.02j\omega)]$, which predicts 99.2 at 1~Hz and 97.0 at 2~Hz, while the filtered droop power reproduces $1/m_{\mathrm{p}}=100$ at every frequency. The inverter's coefficient is flat where the machine's changes sign.
...
In-phase coefficient, 1~Hz, measured in PSCAD & 99.2 (0.50 and 0.60 pu) & $-1.11$ (0.50 pu), $-1.80$ (0.95 pu)\\
...
\caption{Energy-method critical clearing times (navy) compared with PSCAD EMT brackets (orange). Each bracket runs from a successful to an unsuccessful clearing duration; the estimates are single values from the reduced models of Section~\ref{sec:energymethod}: the machine model is the undamped classical one, and the inverter model includes the droop damping $D=1/m_{\mathrm{p}}$. The inverter estimates exceed their brackets because those models carry no current limiter (Section~\ref{sec:gfmgap}).}
\label{fig:efcct}
\end{figure}
