...
\begin{equation}
\Delta\bar{\omega}=m_{\mathrm{p}}(p^\star-\tilde p),\qquad
...
\label{eq:pfilter}
\end{equation}
where $p$ is the unfiltered normalized active power. The controller-generated phase displacement satisfies $\dot\delta_{\mathrm{E}}=\omega_0\Delta\bar{\omega}$ relative to the same synchronous reference. Table~\ref{tab:droopvars} defines the droop variables and their units.

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_basic_model.tex}
\caption{Reduced GFM inverter model, after Figure~1 of the REGFM\_A1 specification \cite{du2023a1}. A controllable voltage source $E\angle\delta_{\mathrm{E}}$ behind coupling reactance $X_{\mathrm{L}}$ delivers active and reactive power to the grid. Under unsaturated filtered droop, $\dot\delta_{\mathrm{E}}=\omega_0\Delta\bar{\omega}$.}
...
\caption{GGOV1 governor and turbine model of the machine, redrawn block for block from Figure~3-5 of IEEE PES-TR1 \cite{pestr1} with the Table~\ref{tab:dyr_gov_params} settings and the figure's signal names; the signal path is described in Section~\ref{sec:gfmmodel}. Grey blocks and wires are present in GGOV1 but switched off by the settings ($K_{\mathrm{imw}}=K_{\mathrm{dgov}}=T_{\mathrm{eng}}=D_{\mathrm{m}}=0$) and carry no signal in any run.}
\label{fig:controlblocks}
\end{figure}

\begin{figure}[tbp]\centering
\input{figures/tikz_gfm_controls.tex}
\caption{REGFM\_A1 outer controls, redrawn from Figures~3(a), 3(b), and~4 of the model specification \cite{du2023a1}: (a)~$P$--$f$ droop with overload mitigation, (b)~$Q$--$V$ droop with the voltage regulator selected by VFlag~$=1$, and (c)~fault current limiting. The inputs have passed through the $T_{\mathrm{Pf}}$, $T_{\mathrm{Qf}}$, and $T_{\mathrm{Vf}}$ measurement filters, and the droop output $\Delta\omega=\omega_0\Delta\bar{\omega}$ is in rad/s.}
\label{fig:gfmblocks}
\end{figure}
...
\begin{align*}
\frac{\mathrm{d}\Delta\bar{\omega}}{\mathrm{d}t}&=-m_{\mathrm{p}}\dot{\tilde p}
=-\frac{m_{\mathrm{p}}}{T_{\mathrm{Pf}}}(p-\tilde p)\\
&=-\frac{m_{\mathrm{p}}}{T_{\mathrm{Pf}}}(p-p^\star)-\frac{\Delta\bar{\omega}}{T_{\mathrm{Pf}}}.
\end{align*}
...
=p^\star-p-\underbrace{\frac{1}{m_{\mathrm{p}}}}_{D_{\mathrm{eq}}}\Delta\bar{\omega},
\qquad
...
\label{eq:govpath}
\end{equation}
where $C(s)=K_{\mathrm{pgov}}+K_{\mathrm{igov}}/s$ is the governor controller ($K_{\mathrm{dgov}}=0$), $A(s)=1/(1+sT_{\mathrm{act}})$ the actuator, $T(s)=(1+sT_{\mathrm{c}})/(1+sT_{\mathrm{b}})$ the turbine lead-lag, $F(s)=1/(1+sT_{\mathrm{pelec}})$ the electrical-power feedback filter selected by Rselect~$=1$, and $W_{\mathrm{f0}}=W_{\mathrm{fnl}}+P_{\mathrm{m0}}/K_{\mathrm{turb}}$ the pre-disturbance fuel flow at dispatch $P_{\mathrm{m0}}$, which enters because Flag~$=1$ makes the fuel flow the product of valve stroke and speed. The electrical power fed back through $F(s)$ is that of a machine swinging against a stiff network, $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}-2Hs\,\Delta\bar{\omega}$ from the swing equation, which gives the factor $(1-2HR_{\mathrm{SG}}sF)$ with $H=5.46$~s. That factor is 1 at zero frequency, so the steady-state value is $1/R_{\mathrm{SG}}=20$ whether or not the rotor's acceleration is included, but at 1~Hz it is $0.47-0.09j$ and removes about half of the direct speed path. 

In the 39-bus fleet $\Delta P_{\mathrm{e}}$ also carries the synchronizing power exchanged with the other nine machines, so Equation~\eqref{eq:govpath} is the single-machine form of the coefficient. $D=0$ in the GENROU record. The five-to-one comparison is therefore $R_{\mathrm{SG}}/m_{\mathrm{p}}=0.05/0.01$ for these normalized coefficients in the zero-frequency limit. It is not a universal ratio of total system damping. The stabilizer contributes nothing to this steady-state comparison: PSS2B acts on its inputs through high-pass (``washout'') stages with time constants $T_{\mathrm{w1}}=T_{\mathrm{w2}}=T_{\mathrm{w3}}=2$~s in the Appendix~\ref{app:params} record, whose gain at zero frequency is zero \cite[Sec.~12.5, p.~770]{kundur1994}, so $1/R_{\mathrm{SG}}$ is the machine's net steady-state droop. At the 0.85--1.0~Hz mode the relevant machine quantities are instead the governor-path value of $-1.5$ to $-1.8$ and the net damping 1.8 in Table~\ref{tab:dh}.

Figure~\ref{fig:damping} plots Equation~\eqref{eq:govpath} against perturbation frequency over the 0.1 to 3~Hz band of electromechanical swings, for the fleet's dispatch range, $P_{\mathrm{m0}}=0.50$ to $1.00$ per unit of each machine's rating (bus~39 at 0.50, the other nine at 0.90--1.00). The governor's job is to hold speed: when the machine speeds up it cuts fuel so that the mechanical power falls, and when the machine slows it adds fuel. For a slow change the power change therefore opposes the speed deviation, which is what damps a swing; the governor is slow, however, because the actuator and the turbine delay the power change it commands. The coefficient starts at 20, falls as the actuator and turbine lags accumulate phase, crosses zero between 0.38 and 0.41~Hz, and reaches $-1.4$ to $-1.8$ at 1~Hz.

Past this frequency the governor's delay exceeds a quarter of the swing period, so the power change it commands arrives after the speed has reversed: mechanical power is then rising while the speed is rising, feeding the swing instead of resisting it. This is negative damping in the sense of \cite[Sec.~2.1, Eq.~(2.2); Sec.~12.5]{kundur1994}, where damping requires a torque that opposes the change in speed. The sign change comes from the two lags in the path, $T_{\mathrm{act}}=0.5$~s and $T_{\mathrm{b}}=0.1$~s; a single first-order lag, the stand-in of \cite[Eq.~(51)]{chatterjee2026note}, never reaches $90^\circ$ and cannot change sign. Closing the feedback with $\Delta P_{\mathrm{e}}=\Delta P_{\mathrm{mech}}$ instead, or omitting the speed dependence of the fuel flow, moves the crossing to 0.6--0.7~Hz and the 1~Hz value to between $-1.1$ and $-2.0$; the sign does not depend on either choice. Damper windings, excitation, the stabilizer, and network interactions also affect the total damping of a mode, and none of them is in Equation~\eqref{eq:govpath}.

The damping coefficient was measured in PSCAD on the single-machine case of Section~\ref{sec:smib} by modulating the infinite-bus frequency, $60+0.05\sin(2\pi f_{\mathrm{m}}t)$~Hz for $f_{\mathrm{m}}=0.3$ to 2~Hz at dispatches of 0.50 and 0.95 per unit, and extracting the component of the mechanical power in phase with the speed deviation at $f_{\mathrm{m}}$. Besides that response, the recorded signals carry a slow drift, as the governor's integrator keeps settling over the window, and small ripple, so the amplitude and phase at $f_{\mathrm{m}}$ are taken from a lock-in fit rather than read off the trace: each recorded signal $x(t)$ is fitted by least squares over the last 10~s of the record, trimmed to whole modulation cycles, as
\[
x(t)\approx c_0+c_1t+c_2t^2+c_3t^3+a_x\cos(2\pi f_{\mathrm{m}}t)+b_x\sin(2\pi f_{\mathrm{m}}t),\qquad \hat X=a_x-jb_x,
\]
where the cubic absorbs the drift, the whole-cycle window keeps it out of the sinusoidal terms, and $\hat X$ is the complex amplitude of the response at $f_{\mathrm{m}}$; the two signals are fitted on the same time axis, so their phases can be subtracted. The coefficient follows from the swing equation, Equation~\eqref{eq:swingeq}, written for small deviations in terms of the per-unit speed deviation,
\[
2H\,\frac{\mathrm{d}\Delta\bar{\omega}}{\mathrm{d}t}=\Delta P_{\mathrm{mech}}-\Delta P_{\mathrm{e}}-D\,\Delta\bar{\omega}.
\]
The damping term is the power proportional to the speed deviation and opposing it, so a device whose power responds to speed contributes $D=-\Delta P/\Delta\bar{\omega}$, positive when its power falls as the speed rises; for the governor that power is the mechanical power. When the speed swings sinusoidally, only the part of the power response in step with the speed acts as this term (the part a quarter cycle out of step is in step with the acceleration and acts as inertia), so the coefficient at $f_{\mathrm{m}}$ is the real part of the ratio of the complex amplitudes,
\begin{equation}
D(f_{\mathrm{m}})=-\operatorname{Re}\left\{\frac{\hat P_{\mathrm{mech}}}{\hat\omega}\right\}
=-\frac{|\hat P_{\mathrm{mech}}|}{|\hat\omega|}\cos(\varphi_P-\varphi_\omega),
\label{eq:dmeas}
\end{equation}
where $\hat P_{\mathrm{mech}}$ and $\hat\omega$ are the complex amplitudes at $f_{\mathrm{m}}$ of the mechanical power and of the per-unit speed deviation $\Delta\bar{\omega}$, $\varphi_P$ and $\varphi_\omega$ their phases, and the sign makes $D$ positive when the mechanical power falls as the speed rises. For the machine, $\hat P_{\mathrm{mech}}$ follows from the recorded mechanical torque, $\hat P_{\mathrm{mech}}=\hat T_{\mathrm{mech}}+T_{\mathrm{m0}}\hat\omega$ in per unit with $T_{\mathrm{m0}}$ the pre-modulation torque; for the inverter, the terminal power replaces $\hat P_{\mathrm{mech}}$ and the internal droop frequency replaces $\hat\omega$. The analytic curves of Figure~\ref{fig:damping} are the same quantity evaluated from the models, $\operatorname{Re}\{G(j2\pi f)/R_{\mathrm{SG}}\}$ from Equation~\eqref{eq:govpath} for the machine and $1/m_{\mathrm{p}}$ for the inverter's filtered power, and the diamond is the net damping $D_{\mathrm{net}}=4H\sigma$ obtained below from the decay rate $\sigma$ of the post-fault oscillation. With the governor deadband removed from a copy of the record, the measured in-phase coefficient follows Equation~\eqref{eq:govpath} within 0.01 at 0.50 per unit ($+2.29$, $-0.86$, $-1.43$, $-1.42$, $-1.10$, and $-0.82$ at 0.3, 0.5, 0.7, 1.0, 1.5, and 2.0~Hz) and within 0.1 at 0.95 per unit, with the zero crossing between 0.3 and 0.5~Hz; these points are plotted in Figure~\ref{fig:damping}. With the supplied deadband of 0.0002 per unit the measured magnitude is 0.2 to 0.3 smaller at the test's 0.0015 per-unit speed swing, because the governor ignores the part of the speed error inside the deadband; at disturbance amplitudes the deadband is not a factor. The same test on the single-inverter case of Section~\ref{sec:smib} (dispatch 0.50 and 0.60 per unit, terminal power projected on the internal droop frequency) gives an in-phase coefficient of 99.9 at 0.3~Hz falling only to 97.0 at 2~Hz; the decrease is the 0.02~s smoothing of the power meter against the 0.01~s power-filter lead, $\mathrm{Re}[(1+j\omega T_{\mathrm{Pf}})/(1+0.02j\omega)]$, which predicts 99.2 at 1~Hz and 97.0 at 2~Hz, while the filtered droop power reproduces $1/m_{\mathrm{p}}=100$ exactly at every frequency; the inverter's coefficient is flat where the machine's changes sign.

The net machine damping in Table~\ref{tab:dh} is measured from the simulated response of the whole machine in the all-machine bus-14 case (experiment \texttt{fault\_3PG\_bus14\_0GFM}, 30~s record, Figure~\ref{fig:allsync}). After clearing, the ten generator-bus active powers oscillate together at the 0.85--1.0~Hz inter-machine mode and the oscillation decays; a Fourier transform of the 4--30~s record gives the frequency, and the logarithm of the ratio of successive peaks gives the decay rate $\sigma=0.06$--$0.10$~s$^{-1}$ for the eight machines away from the fault (buses 31 and 32 carry a faster local component and are excluded). The relation between the decay rate and the damping follows from the swing equation of the lead-in to Equation~\eqref{eq:dmeas} once the electrical power responds to the angle through the synchronizing coefficient, $\Delta P_{\mathrm{e}}=K_{\mathrm{S}}\Delta\delta$, with $\Delta\dot\delta=\omega_0\Delta\bar{\omega}$:
\[
2H\,\frac{\mathrm{d}\Delta\bar{\omega}}{\mathrm{d}t}=-K_{\mathrm{S}}\Delta\delta-D\,\Delta\bar{\omega}
\quad\Longrightarrow\quad
\Delta\ddot\delta+\frac{D}{2H}\,\Delta\dot\delta+\frac{K_{\mathrm{S}}\omega_0}{2H}\,\Delta\delta=0,
\]
the damped oscillator $\ddot x+2\sigma\dot x+\omega_{\mathrm{n}}^2x=0$ with $2\sigma=D/2H$, so $\sigma=D/4H$; the oscillation frequency comes from the last term and does not enter the damping relation \cite[Sec.~12.3]{kundur1994}. The measured decay therefore gives $D_{\mathrm{net}}=4H\sigma\approx1.8$ (1.4--2.2) on the machine base with $H=5.46$~s. The value is net because whatever damps the mode contributes to the measured decay: the governor path, the damper windings, the exciter and stabilizer, and the network interactions all enter, so the coefficient is their total, and its excess over the governor path is not attributed to individual components. The relation is exact for one machine swinging against a stiff system with a constant $D$; for the inter-machine mode of the 39-bus case it gives a per-machine equivalent. PSS2B and ESST4B are active in all machine cases; no case without the stabilizer was simulated. The positive GFM coefficient provides an important interpretation of the less oscillatory response, without attributing every transient difference to that coefficient alone.
...
\caption{Equivalent damping versus perturbation frequency, normalized power per normalized frequency: the GFM filtered-droop coefficient $D_{\mathrm{eq}}=1/m_{\mathrm{p}}=100$ and the machine governor-and-turbine path of Equation~\eqref{eq:govpath} (band: fleet dispatch 0.50 to 1.00 per unit; line: median dispatch; shading: where the path is negative). Points: PSCAD frequency-modulation measurements, Equation~\eqref{eq:dmeas}, on the single-machine case at 0.50 (open circles) and 0.95 per unit (filled squares) and on the single-inverter case at 0.50 and 0.60 per unit (red open circles); diamond: net machine damping from the post-fault oscillation of the all-machine bus-14 case (Table~\ref{tab:dh}).}
\label{fig:damping}
\end{figure}
...
\caption{Energy-method critical clearing times (navy) compared with PSCAD EMT brackets (orange). Each bracket runs from a successful to an unsuccessful clearing duration; the estimates are single values from the reduced models of Section~\ref{sec:energymethod}, and the inverter estimates exceed their brackets because those models carry no current limiter (Section~\ref{sec:gfmgap}).}
\label{fig:efcct}
\end{figure}
