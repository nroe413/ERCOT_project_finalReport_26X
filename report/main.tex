...
\begin{tikzpicture}[
  ...
  stage/.style={box, text width=23mm, minimum height=24mm},
  input/.style={box, text width=41mm, fill=black!8},
  arr/.style={-{Stealth[length=2.6mm]}, line width=0.9pt, color=black}
]
\node[stage] (psse) {\textbf{PSS/E case}\\[1pt]
  {\footnotesize IEEE 39-bus\\ten machines}};
\node[stage, right=15mm of psse] (pscad) {\textbf{PSCAD EMT\\network}\\[1pt]
  {\footnotesize 230~kV,\\Bergeron lines}};
\node[stage, right=5mm of pscad] (run) {\textbf{Automated\\simulations}\\[1pt]
  {\footnotesize Python test scripts\\set faults, dispatch,\\run length}};
\node[stage, right=5mm of run] (ana) {\textbf{Automated\\analysis}\\[1pt]
  {\footnotesize Python scripts\\stitch \texttt{.out} to CSV,\\compute metrics}};
\node[stage, text width=20mm, right=5mm of ana] (figs) {\textbf{Figures and\\analyses}\\[1pt]
  {\footnotesize (Appendix~\ref{app:plotting})}};
\node[input, below=10mm of pscad, xshift=-23mm] (sm)
  {\textbf{Synchronous machine\\dynamics}\\[1pt]
  {\footnotesize ERCOT \texttt{.dyr} record\\(Table~\ref{tab:dyr_gov_params}, Appendix~\ref{app:params})}};
\node[input, below=10mm of pscad, xshift=23mm] (gfm)
  {\textbf{Grid-forming inverter\\dynamics}\\[1pt]
  {\footnotesize PNNL REGFM\_A1 \cite{du2023a1,pnnlgithub}}};
\draw[arr] (psse) -- node[above=1pt, align=center, font=\footnotesize] {E-TRAN\\conversion} (pscad);
\draw[arr] (pscad) -- (run);
\draw[arr] (run) -- (ana);
\draw[arr] (ana) -- (figs);
\draw[arr] (sm.north) -- (pscad.south);
\draw[arr] (gfm.north) -- (pscad.south);
\node[font=\footnotesize, fill=white, inner sep=1.5pt, below=3.5mm of pscad.south] {dropped in};
\end{tikzpicture}
\caption{Testbed workflow. E-TRAN converts the IEEE 39-bus PSS/E case into a PSCAD EMT network; synchronous-machine dynamics from the ERCOT-supplied record and grid-forming inverter dynamics from the PNNL REGFM\_A1 model are dropped into that network; Python test scripts set up and run the simulations, and analysis scripts convert the EMTDC output into tables, metrics, and the report's figures.}
