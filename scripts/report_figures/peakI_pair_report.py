...
DECK_DIR = Path(r"C:\UT_research\NateRoe_ERCOT_Project\.claude\worktrees"
                r"\sharp-jackson-1e89b8\experiments\fault_3PG_bus10_sync_vs_GFM")
...
def main():
    ...
    ag.set_title("grid-forming ($I_{\\mathrm{max}F} = %.1f$ pu)" % deck.IMAXF)
    asy.set_title("synchronous (no current limit)")
    ag.legend(handles=[
        Line2D([], [], color="black", ls="--", lw=1.2,
               label="$I_{\\mathrm{max}F} = %.1f$ pu" % deck.IMAXF),
