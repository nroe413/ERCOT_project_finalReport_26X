...
BENCH = ("origin/gfm_validation_task_2:SOW_task_2/PMView2.4/"
         "pnnlAndNLRBench/4. PSCAD_PSSE_Validation/")
CSV_PNNL = SRC / "A1_VoltUp_PSD_20260513_132718.csv"
CSV_NLR = SRC / "NLR_VoltUp_PSD_20260513_133002.csv"
CSV_PNNL_GIT = BENCH + "2. Voltage Step Up_pnnl/A1_VoltUp_PSD_20260513_132718.csv"
CSV_NLR_GIT = BENCH + "2. Voltage Step Up_NLR/NLR_VoltUp_PSD_20260513_133002.csv"
...
def gitfetch(dest, ref, binary):
    """git-show a blob out of the archive branch (idempotent)."""
    if dest.exists() and dest.stat().st_size > 100:
        return dest
    dest.parent.mkdir(parents=True, exist_ok=True)
    with open(dest, "wb" if binary else "w") as fh:
        subprocess.run(["git", "show", ref], cwd=str(REPO),
                       stdout=fh, check=True)
    print("fetched", dest.name, dest.stat().st_size, "B")
    return dest
...
def make_legacy():
    """Archival raster, copied unchanged -- it has no source data."""
    gitfetch(LVRT_PNG, LVRT_GIT, binary=True)
    dst = OUT / "fig3_mqt_legacy_lvrt.png"
    shutil.copyfile(LVRT_PNG, dst)
    w, h = png_size(dst)
    print("legacy  %s  %d x %d px  (%.0f dpi at width=6.5in)"
          % (dst.name, w, h, w / st.TEXTWIDTH_IN))
    return dst


def make_overlay():
    gitfetch(CSV_PNNL, CSV_PNNL_GIT, binary=True)
    gitfetch(CSV_NLR, CSV_NLR_GIT, binary=True)
