...
and to the inverter's internal droop frequency (experiment
...
def main():
    ...
    ax.plot(scr, h_imp, "o-", color=BLUE, lw=1.6, ms=5, label="referred to the imposed $-1$ Hz/s ramp")
    ...
    ax2.text(2.1, -0.992, "imposed ramp, $-1$ Hz/s", fontsize=7.4, color=BLUE, va="bottom")
    ax2.set_xscale("log")
    ax2.set_xticks(scr); ax2.set_xticklabels(["%g" % s for s in scr])
    ax2.set_xlabel("short-circuit ratio")
    ax2.set_ylabel("slope of the internal frequency (Hz/s)")
