...
def main():
    ...
    axB.annotate("", xy=(1.25, 0), xytext=(0.64, 0),
                 arrowprops=dict(arrowstyle="-|>", lw=0.9, color=INK))
    axB.text(-1.28, 0.13, "current\ndemand", fontsize=8, color=INK,
             ha="left", va="bottom")
    axB.text(1.28, 0.13, "delivered\ncurrent", fontsize=8, color=INK,
             ha="right", va="bottom")
    axB.text(0.52, 0.5, r"$+I_{\max F}$", fontsize=7.5, color=LIM,
             ha="right", va="bottom")
    axB.text(0.52, -0.5, r"$-I_{\max F}$", fontsize=7.5, color=LIM,
             ha="right", va="top")
    axB.text(0, -1.12, "linear until it is not", fontsize=8.5,
    ...
    axP.text(176, PREF + 0.06, r"$P_{\mathrm{ref}}$", fontsize=8,
    ...
    axP.annotate(r"$\delta_I$: limiter binds", xy=(d_I, p_limited(d_I)),
                 xytext=(62, 1.30), fontsize=8, color=INK, ha="left",
                 va="center",
    ...
    axP.text(0.5 * (d_u_lim + d_u_unl), 0.30, "margin lost", fontsize=8,
    ...
    axP.grid(alpha=0.25)
    axP.legend(fontsize=8, loc="upper left", framealpha=0.95,
