...
RUN10 = (WT / "compassionate-banach-aa8040" / "experiments"
         / "fault_3PG_bus14_1GFL" / "runs"
...
def fig8a():
    ...
    s = max(1, len(w) // 24000)
    fig, ax = plt.subplots(figsize=(st.TEXTWIDTH_IN, 3.2))
...
def fig8b():
    ...
    s = max(1, len(w) // 24000)
    t = w.TIME[::s]
    fig, ax = plt.subplots(3, 1, figsize=(st.TEXTWIDTH_IN, 5.8),
                           sharex=True)
    ...
    ax[0].text(3.30, 0.53, r"$V_{dip}=0.5$ (dip-state threshold)",
    ...
    ax[1].plot(t, w.Ipcmd[::s], color=BLUE, lw=1.4,
               label=r"$I_{p,cmd}$ (active)")
    ax[1].plot(t, w.Iqcmd[::s], color=RED, lw=1.4,
               label=r"$I_{q,cmd}$ (reactive)")
