"""Report-format regeneration of narr_inertia_gfm_mp0p1.png (report fig:mp01,
0.8\\textwidth = 5.2 in): the control test - REGFM_A1 with ten-times-weaker
droop (mp = 0.1) collapses the AGS-ESR inertia score to H = 1.22 s (FAIL vs
the 2.5 s criterion), confirming the 7.44 s reading was droop, not inertia.

Data + math reproduced from
sharp-jackson-1e89b8/experiments/agsesr_inertia_mp0p1/inertia_analysis.py
(panel 5), reusing that experiment's _pscad_io loaders on the original PSCAD
run agsesr_freq_inertia_1Hzps_7s (mp = 0.1 variant).  No synthesized data.
"""
import sys
...
def main():
    ...
    t = df["TIME"].values
    P = df["P (Main)"].values                    # per-unit (A1 native)
    ...
    H = 60.0 * abs(dE)
    assert abs(H - 1.22) < 0.02, "H drifted from the published 1.22 s: %.3f" % H
    ...
    fig.savefig(OUT / "narr_inertia_gfm_mp0p1.png", dpi=600)
    print("H = %.3f s  dE = %.5f pu.s  t0 = %.3f s" % (H, dE, t0))
    print("wrote", OUT / "narr_inertia_gfm_mp0p1.png")
