...
| `scripts/report_figures/{fig_mqt_042826, fig3_mqt_split, inertia_timescale, saturation_block, damping_vs_freq, bus14_trip, cct_damping_sweep, hest_scr, ringdown}.py` | `numpy`, `matplotlib` (`pandas` for the two MQT scripts and `bus14_trip`) | nothing else: they run from the shipped CSVs or from constants |
